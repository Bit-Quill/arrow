...
const testString _query = CREATE_STRING("SELECT * FROM ODBCTest.DevOps LIMIT 10000");
...
auto RecordBindingFetching = [](SQLHSTMT& hstmt, std::vector<long long>& times,
...
class TestPerformance : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    ret = SQLDriverConnect(_conn, NULL, &connectionString[0], SQL_NTS, outConnString,
                           HELPER_SIZEOF(outConnString), &outConnStringLen,
                           SQL_DRIVER_COMPLETE);
    if (!SQL_SUCCEEDED(ret)) {
      logDiagnostics(SQL_HANDLE_DBC, _conn, ret);
...
void Report(const std::string& test_case, std::vector<long long> data,
  ...
  outFile << std::to_string(++testNumber) << ","
          ...
          << std::to_string(ITERATION_COUNT) << "," << time_mean << "," << time_max << ","
          << time_min << "," << time_median << "," << percentile << ","
          << averageMemoryUsage << "," << peakMemoryUsage << "\n";
  outFile.close();
}
...
TEST_F(TestPerformance, DISABLED_Time_BindColumn_Fetch5Rows) {
  ...
  boost::thread queryThread = boost::thread([&] {
    ...
    for (size_t iter = 0; iter < ITERATION_COUNT; iter++) {
      ...
      for (size_t i = 0; i < cols.size(); i++) cols[i].resize(ROWSET_SIZE_5);
      ...
      for (size_t i = 0; i < cols.size(); i++)
        ret = SQLBindCol(_hstmt, (SQLUSMALLINT)i + 1, SQL_C_CHAR,
                         ...
                         &cols[i][0].data_len);
      while (SQLExtendedFetch(_hstmt, SQL_FETCH_NEXT, 0, &rows_fetched, row_status) ==
             SQL_SUCCESS) {
        row_count += rows_fetched;
        if (rows_fetched < ROWSET_SIZE_5) break;
      }
      auto end = std::chrono::steady_clock::now();
      ASSERT_TRUE(SQL_SUCCEEDED(SQLCloseCursor(_hstmt)));
      times.push_back(
...
TEST_F(TestPerformance, DISABLED_Time_BindColumn_Fetch50Rows) {
  ...
  boost::thread queryThread = boost::thread([&] {
    ...
    for (size_t iter = 0; iter < ITERATION_COUNT; iter++) {
      ...
      SQLNumResultCols(_hstmt, &total_columns);
      std::vector<std::vector<Col> > cols(total_columns);
      for (size_t i = 0; i < cols.size(); i++) cols[i].resize(ROWSET_SIZE_50);
      ...
      while (SQLExtendedFetch(_hstmt, SQL_FETCH_NEXT, 0, &rows_fetched, row_status) ==
             SQL_SUCCESS) {
        row_count += rows_fetched;
        if (rows_fetched < ROWSET_SIZE_50) break;
      }
...
TEST_F(TestPerformance, DISABLED_Time_Execute_FetchSingleRow) {
  ...
  boost::thread queryThread = boost::thread([&] {
    ...
    for (size_t iter = 0; iter < ITERATION_COUNT; iter++) {
      ...
      SQLNumResultCols(_hstmt, &total_columns);
      std::vector<std::vector<Col> > cols(total_columns);
      for (size_t i = 0; i < cols.size(); i++) cols[i].resize(SINGLE_ROW);
...
TEST_PERF_TEST(WARMING_UP,
               CREATE_STRING("SELECT * FROM perfdb_hcltps.perftable_hcltps LIMIT 10000"),
               true)

TEST_PERF_TEST(
    Q1_EXPECT_49_ROWS,
    CREATE_STRING("SELECT BIN(time, 1m) AS time_bin, AVG(measure_value::double) AS "
...
TEST_PERF_TEST(
    Q2_EXPECT_1_ROW,
    CREATE_STRING("SELECT * FROM perfdb_hcltps.perftable_hcltps WHERE time BETWEEN now() "
...
TEST_PERF_TEST(
    Q3_EXPECT_2_ROWS,
    CREATE_STRING("SELECT BIN(time, 1h) AS hour, COUNT(*) AS num_samples, "
...
TEST_PERF_TEST(
    Q4_EXPECT_1_ROWS,
    CREATE_STRING("WITH gc_timeseries AS ( SELECT region, cell, silo, availability_zone, "
...
TEST_PERF_TEST(
    Q5_EXPECT_2_ROWS,
    CREATE_STRING("SELECT instance_name, BIN(time, 1h) AS time_bin, COUNT(*) AS "
...
TEST_PERF_TEST(
    Q6_EXPECT_1_ROW,
    CREATE_STRING("WITH event_interval AS ( SELECT instance_name, process_name, "
...
TEST_PERF_TEST(
    Q7_EXPECT_49_ROWS,
    CREATE_STRING("SELECT BIN(time, 1m) AS time_bin, AVG(CASE WHEN measure_name = "
...
TEST_PERF_TEST(
    Q8_EXPECT_49_ROWS,
    CREATE_STRING("WITH cpu_user AS ( SELECT BIN(time, 1m) AS time_bin, "
...
TEST_PERF_TEST(
    Q9_EXPECT_2_ROWS,
    CREATE_STRING("SELECT BIN(time, 1h) AS hour, COUNT(CASE WHEN measure_name = "
...
TEST_PERF_TEST(
    Q10_EXPECT_2_ROWS,
    CREATE_STRING("WITH cpu_user AS ( SELECT instance_name, time, measure_value::double "
...
TEST_PERF_TEST(
    Q11_EXPECT_441_ROWS,
    CREATE_STRING("SELECT region, cell, silo, availability_zone, microservice_name, "
...
TEST_PERF_TEST(
    Q12_EXPECT_410_ROWS,
    CREATE_STRING("SELECT region, cell, microservice_name, BIN(time, 1h) AS hour, "
...
TEST_PERF_TEST(
    Q14_EXPECT_10_ROWS,
    CREATE_STRING("WITH per_host_timeseries AS ( SELECT region, cell, silo, "
...
TEST_PERF_TEST(
    Q15_EXPECT_141_ROWS,
    CREATE_STRING("WITH microservice_cell_avg AS ( SELECT region, cell, "
...
TEST_PERF_TEST(
    Q16_EXPECT_505_ROWS,
    CREATE_STRING("WITH per_instance_max_use AS ( SELECT region, cell, silo, "
...
TEST_PERF_TEST(
    Q17_EXPECT_236_ROWS,
    CREATE_STRING("WITH per_instance_memory_used AS ( SELECT region, cell, silo, "
...
TEST_PERF_TEST(
    Q18_EXPECT_410_ROWS,
    CREATE_STRING("WITH per_instance_cpu_used AS ( SELECT region, cell, silo, "
...
TEST_PERF_TEST(
    ...
    CREATE_STRING("WITH task_completed AS ( SELECT region, cell, silo, "
                  "availability_zone, microservice_name, instance_name, process_name, "
...
TEST_PERF_TEST(Q21_EXPECT_15000_ROWS,
               CREATE_STRING("SELECT * FROM perfdb_hcltps.perftable_hcltps LIMIT 15000"),
               true)

TEST_PERF_TEST(
    Q22_EXPECT_1500000_ROWS,
    CREATE_STRING("SELECT * FROM perfdb_hcltps.perftable_hcltps LIMIT 1500000"), true)
...
int main(int argc, char** argv) {
  ...
  if (!accessKeyId.empty() && !secretKey.empty()) {
    specialConnStr = std::string("Driver=Amazon Timestream ODBC Driver;") +
                     "Region=" + region +
                     ";LogLevel=0;Auth=IAM;AccessKeyId=" + accessKeyId + ";" +
                     "SecretKey=" + secretKey + ";";
    connectionString =
