...
ConnectionInfo::ConnectionInfo(const Configuration& config)
  ...
  strParams[SQL_DRIVER_VER] = utility::GetFormatedDriverVersion();
...
#ifdef SQL_COLUMN_ALIAS
  // A character string: "Y" if the data source supports column aliases;
  // otherwise, "N".
  strParams[SQL_COLUMN_ALIAS] = "Y";
#endif  // SQL_COLUMN_ALIAS

#ifdef SQL_IDENTIFIER_QUOTE_CHAR
  // The character string that is used as the starting and ending delimiter of a
...
#ifdef SQL_CATALOG_NAME_SEPARATOR
  // A character string: the character or characters that the data source
  // defines as the separator between a catalog name and the qualified name
  // element that follows or precedes it.
  strParams[SQL_CATALOG_NAME_SEPARATOR] = ".";
#endif  // SQL_CATALOG_NAME_SEPARATOR

#ifdef SQL_SPECIAL_CHARACTERS
  // A character string that contains all special characters (that is, all
...
#ifdef SQL_CATALOG_TERM
  // A character string with the data source vendor's name for a catalog; for
...
#ifdef SQL_COLLATION_SEQ
  // The name of the collation sequence. This is a character string that
  // indicates the name of the default collation for the default character set
...
#ifdef SQL_DATA_SOURCE_NAME
  // A character string with the data source name that was used during
  // connection.
  //
  // If the application called SQLConnect, this is the value of the szDSN
  // argument. If the application called SQLDriverConnect or SQLBrowseConnect,
...
#ifdef SQL_DATA_SOURCE_READ_ONLY
  // A character string. "Y" if the data source is set to READ ONLY mode, "N" if
  // it is otherwise.
  //
  // This characteristic pertains only to the data source itself; it is not a
  // characteristic of the driver that enables access to the data source. A
  ...
  // An SQL-92 Full level-conformant driver will usually return "Y" because it
  // will support the DESCRIBE INPUT statement. Because this does not directly
  // specify the underlying SQL support, however, describing parameters might
  // not be supported, even in a SQL-92 Full level-conformant driver.
...
#ifdef SQL_KEYWORDS
  // A character string that contains a comma-separated list of all data
  // source-specific keywords. This list does not contain keywords specific to
  // ODBC or keywords used by both the data source and ODBC. This list
  ...
  // SQL_MAX_ROW_SIZE information type includes the length of all
  // SQL_LONGVARCHAR and SQL_LONGVARBINARY columns in the row; "N" otherwise.
  strParams[SQL_MAX_ROW_SIZE_INCLUDES_LONG] = "Y";
#endif  // SQL_MAX_ROW_SIZE_INCLUDES_LONG

#ifdef SQL_MULT_RESULT_SETS
  // A character string: "Y" if the data source supports multiple result sets,
  // "N" if it does not.
  ...
  // versions or values for all fetched rows and therefore can detect any
  // updates that were made to a row by any user since the row was last fetched.
  // (This applies only to updates, not to deletions or insertions.) The driver
  // can return the SQL_ROW_UPDATED flag to the row status array when
  // SQLFetchScroll is called. Otherwise, "N"
  strParams[SQL_ROW_UPDATES] = "N";
#endif  // SQL_ROW_UPDATES

#ifdef SQL_SEARCH_PATTERN_ESCAPE
  // A character string specifying what the driver supports as an escape
  // character that allows the use of the pattern match metacharacters
  // underscore (_) and percent sign (%) as valid characters in search patterns.
  // This escape character applies only for those catalog function arguments
  // that support search strings. If this string is empty, the driver does not
  // support a search-pattern escape character. Because this information type
  // does not indicate general support of the escape character in the LIKE
...
#ifdef SQL_ASYNC_DBC_FUNCTIONS
  // Indicates if the driver can execute functions asynchronously on the
  // connection handle. SQL_ASYNC_DBC_CAPABLE = The driver can execute
  // connection functions asynchronously. SQL_ASYNC_DBC_NOT_CAPABLE = The driver
  // can not execute connection functions asynchronously.
  intParams[SQL_ASYNC_DBC_FUNCTIONS] = SQL_ASYNC_DBC_NOT_CAPABLE;
#endif  // SQL_ASYNC_DBC_FUNCTIONS

#ifdef SQL_ASYNC_MODE
  // Indicates the level of asynchronous support in the driver:
  // SQL_AM_CONNECTION = Connection level asynchronous execution is
  // supported.Either all statement handles
  //    associated with a given connection handle are in asynchronous mode or
  //    all are in synchronous mode. A statement handle on a connection cannot
  //    be in asynchronous mode while another statement handle on the same
  //    connection is in synchronous mode, and vice versa.
  // SQL_AM_STATEMENT = Statement level asynchronous execution is supported.Some
  // statement handles
  //    associated with a connection handle can be in asynchronous mode, while
  //    other statement handles on the same connection are in synchronous mode.
  ...
  // SQL_ASYNC_NOTIFICATION_CAPABLE Asynchronous execution notification is
  // supported by the driver. SQL_ASYNC_NOTIFICATION_NOT_CAPABLE Asynchronous
  // execution notification is not supported by the
  //     driver.
  //
  // There are two categories of ODBC asynchronous operations: connection level
  // asynchronous operations and statement level asynchronous operations. If a
  // driver returns SQL_ASYNC_NOTIFICATION_CAPABLE, it must support notification
  // for all APIs that it can execute asynchronously.
  intParams[SQL_ASYNC_NOTIFICATION] = SQL_ASYNC_NOTIFICATION_NOT_CAPABLE;
#endif  // SQL_ASYNC_NOTIFICATION

#ifdef SQL_BATCH_ROW_COUNT
  // Enumerates the behavior of the driver with respect to the availability of
  // row counts. The following bitmasks are used together with the information
  // type: SQL_BRC_ROLLED_UP = Row counts for consecutive INSERT, DELETE, or
  ...
  //     individually available, depending on the SQL_BRC_ROLLED_UP bit.
...
#ifdef SQL_BATCH_SUPPORT
  // Bitmask enumerating the driver's support for batches. The following
  // bitmasks are used to determine which level is supported:
  // SQL_BS_SELECT_EXPLICIT = The driver supports explicit batches that can have
  // result - set generating
  //     statements.
  // SQL_BS_ROW_COUNT_EXPLICIT = The driver supports explicit batches that can
  // have row - count generating
  //     statements.
  // SQL_BS_SELECT_PROC = The driver supports explicit procedures that can have
  // result - set generating
  //     statements.
...
#ifdef SQL_BOOKMARK_PERSISTENCE
  // Bitmask enumerating the operations through which bookmarks persist. The
  // following bitmasks are used together with the flag to determine through
  // which options bookmarks persist: SQL_BP_CLOSE = Bookmarks are valid after
  // an application calls SQLFreeStmt with the SQL_CLOSE option,
  //     or SQLCloseCursor to close the cursor associated with a statement.
  // SQL_BP_DELETE = The bookmark for a row is valid after that row has been
  // deleted. SQL_BP_DROP = Bookmarks are valid after an application calls
  // SQLFreeHandle with a HandleType of
  //     SQL_HANDLE_STMT to drop a statement.
  // SQL_BP_TRANSACTION = Bookmarks are valid after an application commits or
  // rolls back a transaction. SQL_BP_UPDATE = The bookmark for a row is valid
  // after any column in that row has been updated,
  //     including key columns.
  // SQL_BP_OTHER_HSTMT = A bookmark associated with one statement can be used
...
#ifdef SQL_CATALOG_LOCATION
  // Value that indicates the position of the catalog in a qualified table name:
  // SQL_CL_START, SQL_CL_END
  //
  // An SQL - 92 Full level-conformant driver will always return SQL_CL_START.A
  ...
  //     bound column. Note that the columns must be called in order of
  //     ascending column number unless SQL_GD_ANY_ORDER is also returned.
  // SQL_GD_ANY_ORDER = SQLGetData can be called for unbound columns in any
  // order. Note that SQLGetData
  //     can be called only for columns after the last bound column unless
  //     SQL_GD_ANY_COLUMN is also returned.
  // SQL_GD_BLOCK = SQLGetData can be called for an unbound column in any row in
  // a block (where the rowset
  //     size is greater than 1) of data after positioning to that row with
  //     SQLSetPos.
  // SQL_GD_BOUND = SQLGetData can be called for bound columns in addition to
  // unbound columns. A driver
  ...
  // Language statements :
  //     SELECT, INSERT, UPDATE, DELETE, and if supported, SELECT FOR UPDATE and
  //     positioned update and delete statements.
  // SQL_CU_PROCEDURE_INVOCATION = Catalogs are supported in the ODBC procedure
  // invocation statement. SQL_CU_TABLE_DEFINITION = Catalogs are supported in
  // all table definition statements : CREATE TABLE,
  //     CREATE VIEW, ALTER TABLE, DROP TABLE, and DROP VIEW.
  // SQL_CU_INDEX_DEFINITION = Catalogs are supported in all index definition
  ...
  // source.To determine whether catalogs are supported, an application calls
  // SQLGetInfo with the SQL_CATALOG_NAME information type. An SQL - 92 Full
...
#ifdef SQL_SQL92_NUMERIC_VALUE_FUNCTIONS
  // Bitmask enumerating the numeric value scalar functions that are supported
  // by the driver and the associated data source, as defined in SQL-92. The
  // following bitmasks are used to determine which numeric functions are
  // supported : SQL_SNVF_BIT_LENGTH SQL_SNVF_CHAR_LENGTH
  ...
  intParams[SQL_STATIC_CURSOR_ATTRIBUTES2] =
...
#ifdef SQL_PARAM_ARRAY_ROW_COUNTS
  // Enumerating the driver's properties regarding the availability of row
  // counts in a parameterized execution. Has the following values:
  //
  // SQL_PARC_BATCH = Individual row counts are available for each set of
  // parameters. This is conceptually
  //     equivalent to the driver generating a batch of SQL statements, one for
  //     each parameter set in the array. Extended error information can be
  //     retrieved by using the SQL_PARAM_STATUS_PTR descriptor field.
  //
  // SQL_PARC_NO_BATCH = There is only one row count available, which is the
  // cumulative row count
  //     resulting from the execution of the statement for the entire array of
...
#ifdef SQL_PARAM_ARRAY_SELECTS
  // Enumerating the driver's properties regarding the availability of result
  // sets in a parameterized execution. Has the following values:
  //
  // SQL_PAS_BATCH = There is one result set available per set of parameters.
  // This is conceptually
  //     equivalent to the driver generating a batch of SQL statements, one for
  //     each parameter set in the array.
  //
  // SQL_PAS_NO_BATCH = There is only one result set available, which represents
  // the cumulative result set
  //     resulting from the execution of the statement for the complete array of
  //     parameters. This is conceptually equivalent to treating the statement
  //     together with the complete parameter array as one atomic unit.
  //
  ...
  // SQL_SO_KEYSET_DRIVEN = The driver saves and uses the keys for every row in
  // the result set. (ODBC 1.0) SQL_SO_DYNAMIC = The driver keeps the keys for
  // every row in the rowset(the keyset size is the same
  //     as the rowset size). (ODBC 1.0)
  // SQL_SO_MIXED = The driver keeps the keys for every row in the keyset, and
  // the keyset size is greater
  //     than the rowset size.The cursor is keyset - driven inside the keyset
  //     and dynamic outside the keyset. (ODBC 1.0)
  intParams[SQL_SCROLL_OPTIONS] = SQL_SO_FORWARD_ONLY | SQL_SO_STATIC;
#endif  // SQL_SCROLL_OPTIONS

#ifdef SQL_ALTER_DOMAIN
  // Bitmask enumerating the clauses in the ALTER DOMAIN statement, as defined
  // in SQL-92, supported by the data source. An SQL-92 Full level-compliant
  // driver will always return all the bitmasks. A return value of "0" means
  // that the ALTER DOMAIN statement is not supported.
  //
  // The SQL - 92 or FIPS conformance level at which this feature must be
  // supported is shown in parentheses next to each bitmask.
  //
  ...
  // (Full level). SQL_AD_ADD_DOMAIN_DEFAULT = <alter domain> <set domain
  // default clause> is supported (Full level).
  // SQL_AD_CONSTRAINT_NAME_DEFINITION = <constraint name definition clause> is
  // supported for naming
  //     domain constraint (Intermediate level).
  // SQL_AD_DROP_DOMAIN_CONSTRAINT = <drop domain constraint clause> is
  // supported (Full level). SQL_AD_DROP_DOMAIN_DEFAULT = <alter domain> <drop
  // domain default clause> is supported (Full level).
  //
  // The following bits specify the supported <constraint attributes> if <add
  // domain constraint> is supported (the SQL_AD_ADD_DOMAIN_CONSTRAINT bit is
  // set) : SQL_AD_ADD_CONSTRAINT_DEFERRABLE (Full level)
...
#ifdef SQL_ALTER_TABLE
  // Bitmask enumerating the clauses in the ALTER TABLE statement supported by
  // the data source.
  //
  // The SQL - 92 or FIPS conformance level at which this feature must be
  // supported is shown in parentheses next to each bitmask. The following
  // bitmasks are used to determine which clauses are supported :
  // SQL_AT_ADD_COLUMN_COLLATION = <add column> clause is supported, with
  // facility to specify column
  //     collation (Full level) (ODBC 3.0)
  // SQL_AT_ADD_COLUMN_DEFAULT = <add column> clause is supported, with facility
  // to specify column
  //     defaults (FIPS Transitional level) (ODBC 3.0)
  // SQL_AT_ADD_COLUMN_SINGLE = <add column> is supported (FIPS Transitional
  // level) (ODBC 3.0) SQL_AT_ADD_CONSTRAINT = <add column> clause is supported,
  // with facility to specify column
  //     constraints (FIPS Transitional level) (ODBC 3.0)
  // SQL_AT_ADD_TABLE_CONSTRAINT = <add table constraint> clause is
  // supported(FIPS Transitional level)
  //     (ODBC 3.0)
  // SQL_AT_CONSTRAINT_NAME_DEFINITION = <constraint name definition> is
  // supported for naming column and
  //     table constraints(Intermediate level) (ODBC 3.0)
  // SQL_AT_DROP_COLUMN_CASCADE = <drop column> CASCADE is supported (FIPS
  // Transitional level) (ODBC 3.0) SQL_AT_DROP_COLUMN_DEFAULT = <alter column>
  // <drop column default clause> is supported (Intermediate
  //     level) (ODBC 3.0)
  // SQL_AT_DROP_COLUMN_RESTRICT = <drop column> RESTRICT is supported (FIPS
  // Transitional level)
  //     (ODBC 3.0)
  // SQL_AT_DROP_TABLE_CONSTRAINT_CASCADE (ODBC 3.0)
  // SQL_AT_DROP_TABLE_CONSTRAINT_RESTRICT = <drop column> RESTRICT is
  // supported(FIPS Transitional level)
  //     (ODBC 3.0)
  // SQL_AT_SET_COLUMN_DEFAULT = <alter column> <set column default clause> is
  // supported (Intermediate
  //     level) (ODBC 3.0)
  //
  // The following bits specify the support <constraint attributes> if
  // specifying column or table constraints is supported (the
  // SQL_AT_ADD_CONSTRAINT bit is set) : SQL_AT_CONSTRAINT_INITIALLY_DEFERRED
...
#ifdef SQL_CREATE_ASSERTION
  // Bitmask enumerating the clauses in the CREATE ASSERTION statement, as
  // defined in SQL-92, supported by the data source.
  //
  // The following bitmasks are used to determine which clauses are supported :
  // SQL_CA_CREATE_ASSERTION
  //
  // The following bits specify the supported constraint attribute if the
  // ability to specify constraint attributes explicitly is supported(see the
  // SQL_ALTER_TABLE and SQL_CREATE_TABLE information types) :
...
#ifdef SQL_CREATE_CHARACTER_SET
  // Bitmask enumerating the clauses in the CREATE CHARACTER SET statement, as
  // defined in SQL-92, supported by the data source. The following bitmasks are
  // used to determine which clauses are supported :
  ...
  // defined in SQL-92, supported by the data source. The following bitmask is
  // used to determine which clauses are supported : SQL_CCOL_CREATE_COLLATION
...
#ifdef SQL_CREATE_DOMAIN
  // Bitmask enumerating the clauses in the CREATE DOMAIN statement, as defined
  // in SQL-92, supported by the data source. The following bitmasks are used to
  // determine which clauses are supported : SQL_CDO_CREATE_DOMAIN = The CREATE
  // DOMAIN statement is supported(Intermediate level).
  ...
  // SQL_CDO_DEFAULT = Specifying domain constraints is supported (Intermediate
  // level) SQL_CDO_CONSTRAINT = Specifying domain defaults is supported
  // (Intermediate level) SQL_CDO_COLLATION = Specifying domain collation is
  // supported (Full level)
  //
  // The following bits specify the supported constraint attributes if
  // specifying domain constraints is supported (SQL_CDO_DEFAULT is set) :
...
#ifdef SQL_CREATE_SCHEMA
  // Bitmask enumerating the clauses in the CREATE SCHEMA statement, as defined
  // in SQL-92, supported by the data source. The following bitmasks are used to
  // determine which clauses are supported : SQL_CS_CREATE_SCHEMA
  // SQL_CS_AUTHORIZATION
  // SQL_CS_DEFAULT_CHARACTER_SET
  //
  // An SQL - 92 Intermediate level-conformant driver will always return the
  // SQL_CS_CREATE_SCHEMA and SQL_CS_AUTHORIZATION options as supported. These
...
#ifdef SQL_CREATE_TABLE
  // Bitmask enumerating the clauses in the CREATE TABLE statement, as defined
  // in SQL-92, supported by the data source. The SQL - 92 or FIPS conformance
  // level at which this feature must be supported is shown in parentheses next
  // to each bitmask.
  //
  // The following bitmasks are used to determine which clauses are supported :
  // SQL_CT_CREATE_TABLE = The CREATE TABLE statement is supported. (Entry
  // level) SQL_CT_TABLE_CONSTRAINT = Specifying table constraints is supported
  // (FIPS Transitional level) SQL_CT_CONSTRAINT_NAME_DEFINITION = The
  // <constraint name definition> clause is supported for naming
  ...
  // level) SQL_CT_LOCAL_TEMPORARY = Local temporary tables can be created.
  // (Full level)
  //
  // The following bits specify the ability to create column constraints :
  // SQL_CT_COLUMN_CONSTRAINT = Specifying column constraints is supported (FIPS
  // Transitional level) SQL_CT_COLUMN_DEFAULT = Specifying column defaults is
  // supported (FIPS Transitional level) SQL_CT_COLUMN_COLLATION = Specifying
  // column collation is supported (Full level)
  //
  // The following bits specify the supported constraint attributes if
  // specifying column or table constraints is supported :
...
#ifdef SQL_CREATE_VIEW
  // Bitmask enumerating the clauses in the CREATE VIEW statement, as defined in
  ...
  // supported. An SQL - 92 Entry level-conformant driver will always return the
  ...
  // without reflecting any
  //     changes that were made to it by any other cursor within the same
  //     transaction.
  // SQL_UNSPECIFIED = It is unspecified whether cursors on the statement handle
  // make visible the changes
  //     that were made to a result set by another cursor within the same
  //     transaction. Cursors on the statement handle may make visible none,
  //     some, or all such changes.
  // SQL_SENSITIVE = Cursors are sensitive to changes that were made by other
  // cursors within the same
  //     transaction.
  //
...
#ifdef SQL_DEFAULT_TXN_ISOLATION
  // Value that indicates the default transaction isolation level supported by
  // the driver or data source, or zero if the data source does not support
  // transactions. The following terms are used to define transaction isolation
  // levels:
  //
  ...
  // existed. Nonrepeatable Read Transaction 1 reads a row.Transaction 2 updates
  // or deletes that row and commits this change. If transaction 1 tries to
  // reread the row, it will receive different row values or discover that the
  // row has been deleted. Phantom Transaction 1 reads a set of rows that
  // satisfy some search criteria. Transaction 2 generates one or more rows
  // (through either inserts or updates) that match the search criteria. If
  // transaction 1 reexecutes the statement that reads the rows, it receives a
  // different set of rows.
  //
  // If the data source supports transactions, the driver returns one of the
  // following bitmasks : SQL_TXN_READ_UNCOMMITTED = Dirty reads, nonrepeatable
  // reads, and phantoms are possible. SQL_TXN_READ_COMMITTED = Dirty reads are
  // not possible. Nonrepeatable reads and phantoms are possible
  // SQL_TXN_REPEATABLE_READ = Dirty reads and nonrepeatable reads are not
...
#ifdef SQL_DROP_ASSERTION
  // A bitmask enumerating the clauses in the DROP ASSERTION statement, as
  // defined in SQL-92, supported by the data source. The following bitmask is
...
#ifdef SQL_DROP_CHARACTER_SET
  // A bitmask enumerating the clauses in the DROP CHARACTER SET statement, as
  // defined in SQL-92, supported by the data source. The following bitmask is
...
#ifdef SQL_DROP_COLLATION
  // A bitmask enumerating the clauses in the DROP COLLATION statement, as
  // defined in SQL-92, supported by the data source. The following bitmask is
...
#ifdef SQL_DROP_DOMAIN
  // A bitmask enumerating the clauses in the DROP DOMAIN statement, as defined
  // in SQL-92, supported by the data source. The following bitmasks are used to
  // determine which clauses are supported : SQL_DD_DROP_DOMAIN SQL_DD_CASCADE
  // SQL_DD_RESTRICT
...
#ifdef SQL_DROP_SCHEMA
  // A bitmask enumerating the clauses in the DROP SCHEMA statement, as defined
  // in SQL-92, supported by the data source. The following bitmasks are used to
  // determine which clauses are supported : SQL_DS_DROP_SCHEMA SQL_DS_CASCADE
  // SQL_DS_RESTRICT
  // An SQL-92 Intermediate level-conformant driver will always return all of
  // these options as supported.
...
#ifdef SQL_DROP_TABLE
  // A bitmask enumerating the clauses in the DROP TABLE statement, as defined
  // in SQL-92, supported by the data source. The following bitmasks are used to
  // determine which clauses are supported: SQL_DT_DROP_TABLE SQL_DT_CASCADE
  // SQL_DT_RESTRICT
...
#ifdef SQL_DROP_TRANSLATION
  // A bitmask enumerating the clauses in the DROP TRANSLATION statement, as
  // defined in SQL-92, supported by the data source. The following bitmask is
...
#ifdef SQL_DROP_VIEW
  // A bitmask enumerating the clauses in the DROP VIEW statement, as defined in
  // SQL-92, supported by the data source. The following bitmasks are used to
  // determine which clauses are supported: SQL_DV_DROP_VIEW SQL_DV_CASCADE
  // SQL_DV_RESTRICT
  // An FIPS Transitional level-conformant driver will always return all of
  // these options as supported.
  ...
  // supported by the driver. This bitmask contains the first subset of
  // attributes; for the second subset, see SQL_DYNAMIC_CURSOR_ATTRIBUTES2.
  //
  // The following bitmasks are used to determine which attributes are
  // supported: SQL_CA1_NEXT = A FetchOrientation argument of SQL_FETCH_NEXT is
  // supported in a call to SQLFetchScroll
  //     when the cursor is a dynamic cursor.
  // SQL_CA1_ABSOLUTE = FetchOrientation arguments of SQL_FETCH_FIRST,
  // SQL_FETCH_LAST, and
  //     SQL_FETCH_ABSOLUTE are supported in a call to SQLFetchScroll when the
  //     cursor is a dynamic cursor. (The rowset that will be fetched is
  //     independent of the current cursor position.)
  // SQL_CA1_RELATIVE = FetchOrientation arguments of SQL_FETCH_PRIOR and
  // SQL_FETCH_RELATIVE are supported
  //     in a call to SQLFetchScroll when the cursor is a dynamic cursor. (The
  //     rowset that will be fetched depends on the current cursor position.
  //     Note that this is separated from SQL_FETCH_NEXT because in a
  //     forward-only cursor, only SQL_FETCH_NEXT is supported.)
  // SQL_CA1_BOOKMARK = A FetchOrientation argument of SQL_FETCH_BOOKMARK is
  // supported in a call to
  //     SQLFetchScroll when the cursor is a dynamic cursor.
  // SQL_CA1_LOCK_EXCLUSIVE = A LockType argument of SQL_LOCK_EXCLUSIVE is
  // supported in a call to
  //     SQLSetPos when the cursor is a dynamic cursor.
  // SQL_CA1_LOCK_NO_CHANGE = A LockType argument of SQL_LOCK_NO_CHANGE is
  // supported in a call to
  //     SQLSetPos when the cursor is a dynamic cursor.
  // SQL_CA1_LOCK_UNLOCK = A LockType argument of SQL_LOCK_UNLOCK is supported
  // in a call to SQLSetPos
  //     when the cursor is a dynamic cursor.
  // SQL_CA1_POS_POSITION = An Operation argument of SQL_POSITION is supported
  // in a call to SQLSetPos when
  //     the cursor is a dynamic cursor.
  // SQL_CA1_POS_UPDATE = An Operation argument of SQL_UPDATE is supported in a
  // call to SQLSetPos when the
  //     cursor is a dynamic cursor.
  // SQL_CA1_POS_DELETE = An Operation argument of SQL_DELETE is supported in a
  // call to SQLSetPos when the
  //     cursor is a dynamic cursor.
  // SQL_CA1_POS_REFRESH = An Operation argument of SQL_REFRESH is supported in
  // a call to SQLSetPos when
  //     the cursor is a dynamic cursor.
  // SQL_CA1_POSITIONED_UPDATE = An UPDATE WHERE CURRENT OF SQL statement is
  // supported when the cursor is
  //     a dynamic cursor. (An SQL-92 Entry level-conformant driver will always
  //     return this option as supported.)
  // SQL_CA1_POSITIONED_DELETE = A DELETE WHERE CURRENT OF SQL statement is
  // supported when the cursor is a
  //     dynamic cursor. (An SQL-92 Entry level-conformant driver will always
  //     return this option as supported.)
  // SQL_CA1_SELECT_FOR_UPDATE = A SELECT FOR UPDATE SQL statement is supported
  // when the cursor is a
  //     dynamic cursor. (An SQL-92 Entry level-conformant driver will always
  //     return this option as supported.)
  // SQL_CA1_BULK_ADD = An Operation argument of SQL_ADD is supported in a call
  // to SQLBulkOperations when
  //     the cursor is a dynamic cursor.
  // SQL_CA1_BULK_UPDATE_BY_BOOKMARK = An Operation argument of
  // SQL_UPDATE_BY_BOOKMARK is supported in a
  //     call to SQLBulkOperations when the cursor is a dynamic cursor.
  // SQL_CA1_BULK_DELETE_BY_BOOKMARK = An Operation argument of
  // SQL_DELETE_BY_BOOKMARK is supported in a
  //     call to SQLBulkOperations when the cursor is a dynamic cursor.
  // SQL_CA1_BULK_FETCH_BY_BOOKMARK = An Operation argument of
  // SQL_FETCH_BY_BOOKMARK is supported in a
  //     call to SQLBulkOperations when the cursor is a dynamic cursor.
  //
  // An SQL-92 Intermediate level-conformant driver will usually return the
  // SQL_CA1_NEXT, SQL_CA1_ABSOLUTE, and SQL_CA1_RELATIVE options as supported,
  // because it supports scrollable cursors through the embedded SQL FETCH
...
#ifdef SQL_DYNAMIC_CURSOR_ATTRIBUTES2
  // A bitmask that describes the attributes of a dynamic cursor that are
  // supported by the driver. This bitmask contains the second subset of
  // attributes; for the first subset, see SQL_DYNAMIC_CURSOR_ATTRIBUTES1.
  //
  // The following bitmasks are used to determine which attributes are
  // supported: SQL_CA2_READ_ONLY_CONCURRENCY = A read-only dynamic cursor, in
  // which no updates are allowed, is
  //     supported. (The SQL_ATTR_CONCURRENCY statement attribute can be
  //     SQL_CONCUR_READ_ONLY for a dynamic cursor).
  // SQL_CA2_LOCK_CONCURRENCY = A dynamic cursor that uses the lowest level of
  // locking sufficient to make
  //     sure that the row can be updated is supported. (The
  //     SQL_ATTR_CONCURRENCY statement attribute can be SQL_CONCUR_LOCK for a
  //     dynamic cursor.) These locks must be consistent with the transaction
  //     isolation level set by the SQL_ATTR_TXN_ISOLATION connection attribute.
  // SQL_CA2_OPT_ROWVER_CONCURRENCY = A dynamic cursor that uses the optimistic
  // concurrency control
  //     comparing row versions is supported. (The SQL_ATTR_CONCURRENCY
  //     statement attribute can be SQL_CONCUR_ROWVER for a dynamic cursor.)
  // SQL_CA2_OPT_VALUES_CONCURRENCY = A dynamic cursor that uses the optimistic
  // concurrency control
  //     comparing values is supported. (The SQL_ATTR_CONCURRENCY statement
  //     attribute can be SQL_CONCUR_VALUES for a dynamic cursor.)
  // SQL_CA2_SENSITIVITY_ADDITIONS = Added rows are visible to a dynamic cursor;
  // the cursor can scroll to
  //     those rows. (Where these rows are added to the cursor is
  //     driver-dependent.)
  // SQL_CA2_SENSITIVITY_DELETIONS = Deleted rows are no longer available to a
  // dynamic cursor, and do not
  //     leave a "hole" in the result set; after the dynamic cursor scrolls from
  //     a deleted row, it cannot return to that row.
  // SQL_CA2_SENSITIVITY_UPDATES = Updates to rows are visible to a dynamic
  // cursor; if the dynamic cursor
  //     scrolls from and returns to an updated row, the data returned by the
  //     cursor is the updated data, not the original data.
  // SQL_CA2_MAX_ROWS_SELECT = The SQL_ATTR_MAX_ROWS statement attribute affects
  // SELECT statements when
  //     the cursor is a dynamic cursor.
  // SQL_CA2_MAX_ROWS_INSERT = The SQL_ATTR_MAX_ROWS statement attribute affects
  // INSERT statements when
  //     the cursor is a dynamic cursor.
  // SQL_CA2_MAX_ROWS_DELETE = The SQL_ATTR_MAX_ROWS statement attribute affects
  // DELETE statements when
  //     the cursor is a dynamic cursor.
  // SQL_CA2_MAX_ROWS_UPDATE = The SQL_ATTR_MAX_ROWS statement attribute affects
  // UPDATE statements when
  //     the cursor is a dynamic cursor.
  // SQL_CA2_MAX_ROWS_CATALOG = The SQL_ATTR_MAX_ROWS statement attribute
  // affects CATALOG result sets when
  //     the cursor is a dynamic cursor.
  // SQL_CA2_MAX_ROWS_AFFECTS_ALL = The SQL_ATTR_MAX_ROWS statement attribute
  // affects SELECT, INSERT,
  //     DELETE, and UPDATE statements, and CATALOG result sets, when the cursor
  //     is a dynamic cursor.
  // SQL_CA2_CRC_EXACT = The exact row count is available in the
  // SQL_DIAG_CURSOR_ROW_COUNT diagnostic
  //     field when the cursor is a dynamic cursor.
  // SQL_CA2_CRC_APPROXIMATE = An approximate row count is available in the
  // SQL_DIAG_CURSOR_ROW_COUNT
  //     diagnostic field when the cursor is a dynamic cursor.
  // SQL_CA2_SIMULATE_NON_UNIQUE = The driver does not guarantee that simulated
  // positioned update or
  //     delete statements will affect only one row when the cursor is a dynamic
  //     cursor; it is the application's responsibility to guarantee this. (If a
  //     statement affects more than one row, SQLExecute or SQLExecDirect
  //     returns SQLSTATE 01001 [Cursor operation conflict].) To set this
  //     behavior, the application calls SQLSetStmtAttr with the
  //     SQL_ATTR_SIMULATE_CURSOR attribute set to SQL_SC_NON_UNIQUE.
  // SQL_CA2_SIMULATE_TRY_UNIQUE = The driver tries to guarantee that simulated
  // positioned update or
  //     delete statements will affect only one row when the cursor is a dynamic
  //     cursor. The driver always executes such statements, even if they might
  //     affect more than one row, such as when there is no unique key. (If a
  //     statement affects more than one row, SQLExecute or SQLExecDirect
  //     returns SQLSTATE 01001 [Cursor operation conflict].) To set this
  //     behavior, the application calls SQLSetStmtAttr with the
  //     SQL_ATTR_SIMULATE_CURSOR attribute set to SQL_SC_TRY_UNIQUE.
  // SQL_CA2_SIMULATE_UNIQUE = The driver guarantees that simulated positioned
  // update or delete statements
  //     will affect only one row when the cursor is a dynamic cursor. If the
  //     driver cannot guarantee this for a given statement, SQLExecDirect or
...
#ifdef SQL_INDEX_KEYWORDS
  // A bitmask that enumerates keywords in the CREATE INDEX statement that are
...
#ifdef SQL_INFO_SCHEMA_VIEWS
  // A bitmask enumerating the views in the INFORMATION_SCHEMA that are
  // supported by the driver. The views in, and the contents of,
  // INFORMATION_SCHEMA are as defined in SQL-92. The SQL-92 or FIPS conformance
  // level at which this feature must be supported is shown in parentheses next
  // to each bitmask.
  //
  // The following bitmasks are used to determine which views are supported:
  // SQL_ISV_ASSERTIONS = Identifies the catalog's assertions that are owned by
  // a given user. (Full level) SQL_ISV_CHARACTER_SETS = Identifies the
  // catalog's character sets that can be accessed by a given
  //     user. (Intermediate level)
  // SQL_ISV_CHECK_CONSTRAINTS = Identifies the CHECK constraints that are owned
  // by a given user.
  //     (Intermediate level)
  // SQL_ISV_COLLATIONS = Identifies the character collations for the catalog
  // that can be accessed by a
  //     given user. (Full level)
  // SQL_ISV_COLUMN_DOMAIN_USAGE = Identifies columns for the catalog that
  // depend on domains defined in
  //     the catalog and are owned by a given user. (Intermediate level)
  // SQL_ISV_COLUMN_PRIVILEGES = Identifies the privileges on columns of
  // persistent tables that are
  //     available to or granted by a given user. (FIPS Transitional level)
  // SQL_ISV_COLUMNS = Identifies the columns of persistent tables that can be
  // accessed by a given user.
  //     (FIPS Transitional level)
  // SQL_ISV_CONSTRAINT_COLUMN_USAGE = Similar to CONSTRAINT_TABLE_USAGE view,
  // columns are identified for
  //     the various constraints that are owned by a given user. (Intermediate
  //     level)
  // SQL_ISV_CONSTRAINT_TABLE_USAGE = Identifies the tables that are used by
  // constraints (referential,
  //     unique, and assertions), and are owned by a given user. (Intermediate
  //     level)
  // SQL_ISV_DOMAIN_CONSTRAINTS = Identifies the domain constraints (of the
  // domains in the catalog) that
  //     can be accessed by a given user. (Intermediate level)
  // SQL_ISV_DOMAINS = Identifies the domains defined in a catalog that can be
  // accessed by the user.
  //     (Intermediate level)
  // SQL_ISV_KEY_COLUMN_USAGE = Identifies columns defined in the catalog that
  // are constrained as keys
  //     by a given user. (Intermediate level)
  // SQL_ISV_REFERENTIAL_CONSTRAINTS = Identifies the referential constraints
  // that are owned by a given
  //     user. (Intermediate level)
  // SQL_ISV_SCHEMATA = Identifies the schemas that are owned by a given user.
  // (Intermediate level) SQL_ISV_SQL_LANGUAGES = Identifies the SQL conformance
  // levels, options, and dialects supported by
  //     the SQL implementation. (Intermediate level)
  // SQL_ISV_TABLE_CONSTRAINTS = Identifies the table constraints that are owned
  // by a given user.
  //     (Intermediate level)
  // SQL_ISV_TABLE_PRIVILEGES = Identifies the privileges on persistent tables
  // that are available to or
  //     granted by a given user. (FIPS Transitional level)
  // SQL_ISV_TABLES = Identifies the persistent tables defined in a catalog that
  // can be accessed by a
  //     given user. (FIPS Transitional level)
  // SQL_ISV_TRANSLATIONS = Identifies character translations for the catalog
  // that can be accessed by a
  //     given user. (Full level)
  // SQL_ISV_USAGE_PRIVILEGES = Identifies the USAGE privileges on catalog
  // objects that are available to
  //     or owned by a given user. (FIPS Transitional level)
  // SQL_ISV_VIEW_COLUMN_USAGE = Identifies the columns on which the catalog's
  // views that are owned by a
  //     given user are dependent. (Intermediate level)
  // SQL_ISV_VIEW_TABLE_USAGE = Identifies the tables on which the catalog's
  // views that are owned by a
  //     given user are dependent. (Intermediate level)
  ...
  // supported by the driver. This bitmask contains the first subset of
  // attributes; for the second subset, see SQL_KEYSET_CURSOR_ATTRIBUTES2.
  //
  // The following bitmasks are used to determine which attributes are
  // supported: SQL_CA1_NEXT SQL_CA1_ABSOLUTE SQL_CA1_RELATIVE SQL_CA1_BOOKMARK
  // SQL_CA1_LOCK_EXCLUSIVE
  // SQL_CA1_LOCK_NO_CHANGE
  // SQL_CA1_LOCK_UNLOCK
  // SQL_CA1_POS_POSITION
  ...
  // An SQL-92 Intermediate level-conformant driver will usually return the
  // SQL_CA1_NEXT, SQL_CA1_ABSOLUTE, and SQL_CA1_RELATIVE options as supported,
  // because the driver supports scrollable cursors through the embedded SQL
  ...
  // A bitmask that describes the attributes of a keyset cursor that are
  // supported by the driver. This bitmask contains the second subset of
  // attributes; for the first subset, see SQL_KEYSET_CURSOR_ATTRIBUTES1.
  //
  // The following bitmasks are used to determine which attributes are
...
#ifdef SQL_MAX_BINARY_LITERAL_LEN
  // Value that specifies the maximum length (number of hexadecimal characters,
  // excluding the literal prefix and suffix returned by SQLGetTypeInfo) of a
...
#ifdef SQL_MAX_CATALOG_NAME_LEN
  // Value that specifies the maximum length of a catalog name in the data
...
#ifdef SQL_MAX_CHAR_LITERAL_LEN
  // Value that specifies the maximum length (number of characters, excluding
...
#ifdef SQL_MAX_ROW_SIZE
  // Value that specifies the maximum length of a single row in a table. If
...
#ifdef SQL_SQL92_FOREIGN_KEY_DELETE_RULE
  // A bitmask enumerating the rules supported for a foreign key in a DELETE
  // statement, as defined in SQL-92.
  //
  // The following bitmasks are used to determine which clauses are supported by
  // the data source: SQL_SFKD_CASCADE SQL_SFKD_NO_ACTION SQL_SFKD_SET_DEFAULT
  // SQL_SFKD_SET_NULL
  //
  // An FIPS Transitional level-conformant driver will always return all of
  // these options as supported.
  ...
  //
  // The following bitmasks are used to determine which clauses are supported by
  // the data source: SQL_SFKU_CASCADE SQL_SFKU_NO_ACTION SQL_SFKU_SET_DEFAULT
  // SQL_SFKU_SET_NULL
  //
  // An SQL-92 Full level-conformant driver will always return all of these
  // options as supported.
  ...
  // defined in SQL-92. The SQL-92 or FIPS conformance level at which this
  // feature must be supported is shown in parentheses next to each bitmask. The
  // following bitmasks are used to determine which clauses are supported by the
  // data source: SQL_SG_DELETE_TABLE (Entry level) SQL_SG_INSERT_COLUMN
  // (Intermediate level) SQL_SG_INSERT_TABLE (Entry level)
...
#ifdef SQL_SQL92_REVOKE
  // A bitmask enumerating the clauses supported in the REVOKE statement, as
  // defined in SQL-92, supported by the data source. The SQL-92 or FIPS
  // conformance level at which this feature must be supported is shown in
  // parentheses next to each bitmask.
  //
  // The following bitmasks are used to determine which clauses are supported by
  // the data source: SQL_SR_CASCADE (FIPS Transitional level)
...
#ifdef SQL_SQL92_ROW_VALUE_CONSTRUCTOR
  // A bitmask enumerating the row value constructor expressions supported in a
  // SELECT statement, as defined in SQL-92. The following bitmasks are used to
  // determine which options are supported by the data source:
...
#ifdef SQL_STANDARD_CLI_CONFORMANCE
  // A bitmask enumerating the CLI standard or standards to which the driver
  // conforms. The following bitmasks are used to determine which levels the
  ...
  // SQL_SQ_QUANTIFIED
  //
  // The SQL_SQ_CORRELATED_SUBQUERIES bitmask indicates that all predicates that
  ...
  //
  // For descriptions of these isolation levels, see the description of
  // SQL_DEFAULT_TXN_ISOLATION. To set the transaction isolation level, an
  // application calls SQLSetConnectAttr to set the SQL_ATTR_TXN_ISOLATION
  // attribute. For more information, see SQLSetConnectAttr Function. An SQL-92
  ...
  // SQL_U_UNION_ALL = The data source supports the ALL keyword in the UNION
  // clause. (SQLGetInfo returns
...
#ifdef SQL_FETCH_DIRECTION
  // DEPRECATED. Included for backward-compatibility.
  // The information type was introduced in ODBC 1.0; each bitmask is labeled
  // with the version in which it was introduced. A bitmask enumerating the
  // supported fetch direction options: SQL_FD_FETCH_NEXT (ODBC 1.0)
...
#ifdef SQL_LOCK_TYPES
  // DEPRECATED. Included for backward-compatibility.
  // A bitmask enumerating the supported lock types for the fLock argument in
  // SQLSetPos: SQL_LCK_NO_CHANGE SQL_LCK_EXCLUSIVE SQL_LCK_UNLOCK
  ...
  // SQL_SCCO_OPT_VALUES = Cursor uses optimistic concurrency control, comparing
  // values.
...
#ifdef SQL_STATIC_SENSITIVITY
  // DEPRECATED. Included for backward-compatibility.
  // A bitmask enumerating whether changes made by an application to a static or
  // keyset-driven cursor through SQLSetPos or positioned update or delete
  // statements can be detected by that application.
  //
  // Whether an application can detect changes made to the result set by other
  // users, including other cursors in the same application, depends on the
  // cursor type.
  //
  // SQL_SS_ADDITIONS = Added rows are visible to the cursor; the cursor can
  // scroll to these rows.
  //    Where these rows are added to the cursor is driver-dependent.
  // SQL_SS_DELETIONS = Deleted rows are no longer available to the cursor and
  // do not leave a "hole" in
  //   the result set; after the cursor scrolls from a deleted row, it cannot
  //   return to that row.
  // SQL_SS_UPDATES = Updates to rows are visible to the cursor; if the cursor
  // scrolls from and returns to
  //    an updated row, the data returned by the cursor is the updated data, not
  //    the original data. This option applies only to static cursors or updates
  ...
  // specification with which the version of the ODBC Driver Manager fully
  // complies.
...
#ifdef SQL_CONCAT_NULL_BEHAVIOR
  // Indicates how the data source handles the concatenation of NULL valued
  // character data type columns with non-NULL valued character data type
  ...
  // SQL_CN_DIFFERENT = Correlation names are supported but must differ from the
  // names of the tables they
  //     represent.
  // SQL_CN_ANY = Correlation names are supported and can be any valid user -
  // defined name. An SQL - 92 Entry level-conformant driver will always return
  // SQL_CN_ANY.
  shortParams[SQL_CORRELATION_NAME] = SQL_CN_ANY;
#endif  // SQL_CORRELATION_NAME

#ifdef SQL_FILE_USAGE
  // Value that indicates how a single-tier driver directly treats files in a
  // data source:
  //
  // SQL_FILE_NOT_SUPPORTED = The driver is not a single-tier driver. For
  // example, an ORACLE driver is a
  //     two-tier driver.
  // SQL_FILE_TABLE = A single-tier driver treats files in a data source as
  // tables. For example, an Xbase
  //     driver treats each Xbase file as a table.
  // SQL_FILE_CATALOG = A single-tier driver treats files in a data source as a
  // catalog. For example, a
  //     Microsoft Access driver treats each Microsoft Access file as a complete
  //     database.
  //
  // An application might use this to determine how users will select data. For
  // example, Xbase users often think of data as stored in files, whereas ORACLE
  // and Microsoft Access users generally think of data as stored in tables.
  //
  // When a user selects an Xbase data source, the application could display the
  // Windows File Open common dialog box; when the user selects a Microsoft
  // Access or ORACLE data source, the application could display a custom Select
  // Table dialog box.
  shortParams[SQL_FILE_USAGE] = SQL_FILE_NOT_SUPPORTED;
#endif  // SQL_FILE_USAGE

#ifdef SQL_GROUP_BY
  // Value that specifies the relationship between the columns in the GROUP BY
  ...
  //     (ODBC 2.0)
  // SQL_GB_GROUP_BY_CONTAINS_SELECT = The GROUP BY clause must contain all
  // nonaggregated columns in the
  //     select list. It can contain columns that are not in the select list.
  //     For example, SELECT DEPT, MAX(SALARY) FROM EMPLOYEE GROUP BY DEPT, AGE.
  //     (ODBC 2.0)
  // SQL_GB_NO_RELATION = The columns in the GROUP BY clause and the select list
  // are not related.
  //     The meaning of nongrouped, nonaggregated columns in the select list is
  //     data source-dependent. For example, SELECT DEPT, SALARY FROM EMPLOYEE
  //     GROUP BY DEPT, AGE. (ODBC 2.0)
  //
  // An SQL-92 Entry level-conformant driver will always return the
  // SQL_GB_GROUP_BY_EQUALS_SELECT option as supported. An SQL-92 Full
  ...
  // Value as follows:
  //
  // SQL_IC_UPPER = Identifiers in SQL are not case-sensitive and are stored in
  // uppercase in system
  //     catalog.
  // SQL_IC_LOWER = Identifiers in SQL are not case-sensitive and are stored in
  // lowercase in system
  //     catalog.
  // SQL_IC_SENSITIVE = Identifiers in SQL are case sensitive and are stored in
  // mixed case in system
  //     catalog.
  // SQL_IC_MIXED = Identifiers in SQL are not case-sensitive and are stored in
  // mixed case in system
  //     catalog.
  //
...
#ifdef SQL_MAX_COLUMNS_IN_GROUP_BY
  // Value that specifies the maximum number of columns allowed in a GROUP BY
  // clause. If there is no specified limit or the limit is unknown, this value
  // is set to zero. An FIPS Entry level-conformant driver will return at
...
#ifdef SQL_MAX_COLUMNS_IN_ORDER_BY
  // Value that specifies the maximum number of columns allowed in an ORDER BY
  // clause. If there is no specified limit or the limit is unknown, this value
  // is set to zero. An FIPS Entry level-conformant driver will return at
  // least 6. An FIPS Intermediate level-conformant driver will return at
  // least 15.
...
#ifdef SQL_MAX_COLUMNS_IN_SELECT
  // Value that specifies the maximum number of columns allowed in a select
...
#ifdef SQL_MAX_COLUMNS_IN_TABLE
  // Value that specifies the maximum number of columns allowed in a table. If
  // there is no specified limit or the limit is unknown, this value is set to
...
#ifdef SQL_MAX_CURSOR_NAME_LEN
  // Value that specifies the maximum length of a cursor name in the data
  // source. If there is no maximum length or the length is unknown, this value
  // is set to zero. An FIPS Entry level-conformant driver will return at
  // least 18. An FIPS Intermediate level-conformant driver will return at least
  // 128.
...
#ifdef SQL_MAX_DRIVER_CONNECTIONS
  // Value that specifies the maximum number of active connections that the
  // driver can support for an environment. This value can reflect a limitation
  // imposed by either the driver or the data source. If there is no specified
...
#ifdef SQL_MAX_IDENTIFIER_LEN
  // Value that indicates the maximum size in characters that the data source
...
#ifdef SQL_MAX_TABLE_NAME_LEN
  // Value that specifies the maximum length of a table name in the data source.
...
#ifdef SQL_MAX_TABLES_IN_SELECT
  // Value that specifies the maximum number of tables allowed in the FROM
  // clause of a SELECT statement. If there is no specified limit or the limit
  ...
  // SQL_NNC_NON_NULL = Columns cannot be nullable. (The data source supports
  // the NOT NULL column
  ...
  // SQL_NC_END = NULLs are sorted at the end of the result set, regardless of
  // the ASC or DESC keywords. SQL_NC_HIGH = NULLs are sorted at the high end of
  // the result set, depending on the ASC or DESC
  //     keywords.
  // SQL_NC_LOW = NULLs are sorted at the low end of the result set, depending
  // on the ASC or DESC
  //     keywords.
