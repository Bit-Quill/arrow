...
BigInteger::BigInteger(int64_t val) : sign(1), mag() { AssignInt64(val); }

BigInteger::BigInteger(const char* val, int32_t len) : sign(1), mag() {
  AssignString(val, len);
}

BigInteger::BigInteger(const BigInteger& other) : sign(other.sign), mag(other.mag) {
  // No-op.
}
...
BigInteger::BigInteger(const int8_t* val, int32_t len, int32_t sign, bool bigEndian)
  ...
  if (bigEndian) {
    ...
    while (firstNonZero < len && val[firstNonZero] == 0) ++firstNonZero;
    ...
    for (int32_t i = 0; i < intLength - 1; ++i) {
      mag[i] = (val[b] & 0xFFUL) | ((val[b - 1] & 0xFFUL) << 8) |
               ((val[b - 2] & 0xFFUL) << 16) | ((val[b - 3] & 0xFFUL) << 24);
    ...
    while (firstNonZero >= 0 && val[firstNonZero] == 0) --firstNonZero;
...
BigInteger::BigInteger(MagArray& mag, int8_t sign) : sign(sign), mag() {
  this->mag.Swap(mag);
}
...
int8_t BigInteger::GetSign() const { return sign; }
...
const BigInteger::MagArray& BigInteger::GetMagnitude() const { return mag; }
...
uint32_t BigInteger::GetBitLength() const {
  ...
  if (mag.GetSize() > 1) res += (mag.GetSize() - 1) * 32;
...
void BigInteger::Pow(int32_t exp) {
  ...
  int32_t mutExp = exp;
  while (mutExp) {
    if (mutExp & 1) Multiply(multiplicant, *this);

    mutExp >>= 1;

    if (mutExp) multiplicant.Multiply(multiplicant, multiplicant);
  }
}
...
void BigInteger::Multiply(const BigInteger& other, BigInteger& res) const {
  ...
  for (int32_t i = 0; i < other.mag.GetSize(); ++i) {
    ...
    for (int32_t j = 0; j < mag.GetSize(); ++j) {
      uint64_t product =
          static_cast<uint64_t>(mag[j]) * other.mag[i] + +resMag[i + j] + carry;
...
void ShiftLeft(const uint32_t* in, int32_t len, uint32_t* out, unsigned n) {
  ...
  for (int32_t i = len - 1; i > 0; --i) out[i] = (in[i] << n) | (in[i - 1] >> (32 - n));
...
void ShiftRight(const uint32_t* in, int32_t len, uint32_t* out, unsigned n) {
  ...
  for (int32_t i = 0; i < len - 1; ++i) out[i] = (in[i] >> n) | (in[i + 1] << (32 - n));
...
uint32_t BigInteger::GetMagInt(int32_t n) const {
  ...
  if (n >= mag.GetSize()) return sign > 0 ? 0 : -1;
...
void BigInteger::Divide(const BigInteger& divisor, BigInteger& res,
  ...
  if (mag.GetSize() <= 2) {
    ...
    if (mag.GetSize() == 2) u |= static_cast<uint64_t>(mag[1]) << 32;

    if (divisor.mag.GetSize() == 2) v |= static_cast<uint64_t>(divisor.mag[1]) << 32;
    ...
    // (u % v) < (u / 2).
    if (rem) rem->AssignInt64(resSign * static_cast<int64_t>(u % v));
  ...
  for (int32_t i = ulen - vlen; i >= 0; --i) {
    ...
    while (qhat > UINT32_MAX ||
           ((qhat * nv[vlen - 2]) >
            ((UINT32_MAX + static_cast<uint64_t>(1)) * rhat + nu[i + vlen - 2]))) {
      --qhat;
      rhat += nv[vlen - 1];
...
void BigInteger::Normalize() {
  ...
  while (lastNonZero >= 0 && mag[lastNonZero] == 0) --lastNonZero;
