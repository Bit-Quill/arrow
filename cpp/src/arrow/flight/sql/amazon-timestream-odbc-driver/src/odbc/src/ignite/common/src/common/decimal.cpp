...
Decimal::Decimal(const int8_t* mag, int32_t len, int32_t scale, int32_t sign,
                 bool bigEndian)
    : scale(scale & 0x7FFFFFFF), magnitude(mag, len, sign, bigEndian) {
  // No-op.
}
...
Decimal::Decimal(int64_t val) : scale(0), magnitude(val) {
  // No-op.
}

Decimal::Decimal(int64_t val, int32_t scale) : scale(scale), magnitude(val) {
  // No-op.
}
...
Decimal::Decimal(const char* val, int32_t len) : scale(0), magnitude(0) {
