...
BOOST_AUTO_TEST_CASE(TestSQLBindParameter) {
  ...
  SQLRETURN ret = SQLBindParameter(stmt, 1, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0,
                                   0, &int1, 0, &len1);
...
BOOST_AUTO_TEST_CASE(TestSQLSetDescRec) {
  ...
  ret = SQLSetDescRec(desc, 2, SQL_INTEGER, 0, 0, 0, 0, (SQLPOINTER)&data, NULL, NULL);
  BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
...
BOOST_AUTO_TEST_CASE(TestSQLGetDescRec) {
  ...
  ret =
      SQLGetDescRec(desc, 1, column.data(), 10, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
  BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
  CheckSQLDiagnosticError(SQL_HANDLE_DESC, desc, "HYC00");
...
BOOST_AUTO_TEST_CASE(TestSQLSetCursorNameTruncated) {
  ...
  BOOST_CHECK_EQUAL(timestream::odbc::utility::SqlWcharToString(cursorNameRes), "curso");
...
BOOST_AUTO_TEST_CASE(TestSQLSetCursorNameMultipleTimes) {
  ...
  BOOST_CHECK_EQUAL(timestream::odbc::utility::SqlWcharToString(cursorName2.data()),
                    timestream::odbc::utility::SqlWcharToString(cursorNameRes));
  BOOST_CHECK_EQUAL(resLen, 7);
}
