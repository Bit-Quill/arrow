...
BOOST_AUTO_TEST_CASE(TestSQLDriverConnect) {
  ...
  // Normal connect.
  SQLRETURN ret = SQLDriverConnect(dbc, NULL, &connectStr[0], connectStrLen, outStr,
                                   ODBC_BUFFER_SIZE, &outStrLen, SQL_DRIVER_COMPLETE);
  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);
  SQLDisconnect(dbc);

  // Null out string resulting length.
  SQLDriverConnect(dbc, NULL, &connectStr[0], connectStrLen, outStr, ODBC_BUFFER_SIZE, 0,
                   SQL_DRIVER_COMPLETE);
  SQLDisconnect(dbc);

  // Null out string buffer length.
  SQLDriverConnect(dbc, NULL, &connectStr[0], connectStrLen, outStr, 0, &outStrLen,
                   SQL_DRIVER_COMPLETE);
  SQLDisconnect(dbc);

  // Null out string.
  SQLDriverConnect(dbc, NULL, &connectStr[0], connectStrLen, 0, ODBC_BUFFER_SIZE,
...
BOOST_AUTO_TEST_CASE(TestSQLPrepare) {
  ...
  std::vector<SQLWCHAR> sql = MakeSqlBuffer("SELECT * FROM \"sampleDB\".IoTMulti");
...
BOOST_AUTO_TEST_CASE(TestSQLTables) {
  ...
  // Everything is ok.
  SQLRETURN ret = SQLTables(stmt, catalogName.data(), SQL_NTS, schemaName.data(), SQL_NTS,
                            tableName.data(), SQL_NTS, tableType.data(), SQL_NTS);
  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);

  // Sizes are nulls.
  ret = SQLTables(stmt, catalogName.data(), 0, schemaName.data(), 0, tableName.data(), 0,
                  tableType.data(), 0);
  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);
...
BOOST_AUTO_TEST_CASE(TestSQLColumns) {
  ...
  // Sizes are nulls.
  ret = SQLColumns(stmt, catalogName.data(), 0, schemaName.data(), 0, tableName.data(), 0,
                   columnName.data(), 0);
  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);
...
BOOST_AUTO_TEST_CASE(TestSQLColAttribute) {
  ...
  ret = SQLColAttribute(stmt, 1, SQL_COLUMN_TABLE_NAME, buffer, sizeof(buffer), &resLen,
                        &numericAttr);

  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);

  // Everything is ok. Numeric attribute.
  ret = SQLColAttribute(stmt, 1, SQL_DESC_COUNT, buffer, sizeof(buffer), &resLen,
                        &numericAttr);

  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);

  SQLColAttribute(stmt, 1, SQL_DESC_BASE_COLUMN_NAME, buffer, sizeof(buffer), &resLen, 0);
#ifndef __APPLE__
  ...
  SQLColAttribute(stmt, 1, SQL_DESC_BASE_COLUMN_NAME, buffer, sizeof(buffer), 0,
                  &numericAttr);
  SQLColAttribute(stmt, 1, SQL_DESC_BASE_COLUMN_NAME, buffer, sizeof(buffer), 0, 0);
#endif  // __APPLE__
  SQLColAttribute(stmt, 1, SQL_DESC_BASE_COLUMN_NAME, buffer, 0, &resLen, &numericAttr);
...
BOOST_AUTO_TEST_CASE(TestSQLDescribeCol) {
  ...
  // Everything is ok.
  ret = SQLDescribeCol(stmt, 1, columnName, ODBC_BUFFER_SIZE, &columnNameLen, &dataType,
                       &columnSize, &decimalDigits, &nullable);
  ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);

  // Confirm boundary condition.
  SQLSMALLINT reducedNameLen = 4;
  ret = SQLDescribeCol(stmt, 1, columnName, reducedNameLen + 1, &columnNameLen, &dataType,
                       &columnSize, &decimalDigits, &nullable);
  ...
  ret = SQLDescribeCol(stmt, 1, columnName, 0, &columnNameLen, &dataType, &columnSize,
                       &decimalDigits, &nullable);
  BOOST_CHECK_EQUAL(ret, SQL_SUCCESS);
  BOOST_CHECK_EQUAL(columnNameLen, 0);

  ret = SQLDescribeCol(stmt, 1, columnName, ODBC_BUFFER_SIZE, 0, &dataType, &columnSize,
                       &decimalDigits, &nullable);
  BOOST_CHECK_EQUAL(ret, SQL_SUCCESS);
  ...
  ret = SQLDescribeCol(stmt, 1, columnName, ODBC_BUFFER_SIZE, &columnNameLen, &dataType,
                       0, &decimalDigits, &nullable);
  BOOST_CHECK_EQUAL(ret, SQL_SUCCESS);

  ret = SQLDescribeCol(stmt, 1, columnName, ODBC_BUFFER_SIZE, &columnNameLen, &dataType,
                       &columnSize, 0, &nullable);
  BOOST_CHECK_EQUAL(ret, SQL_SUCCESS);
...
BOOST_AUTO_TEST_CASE(TestSQLGetStmtAttr) {
  ...
  // Everything is ok.
  SQLRETURN ret =
      SQLGetStmtAttr(stmt, SQL_ATTR_ROW_ARRAY_SIZE, buffer, sizeof(buffer), &resLen);
...
BOOST_AUTO_TEST_CASE(TestSQLGetDiagField) {
  ...
  SQLGetDiagField(SQL_HANDLE_STMT, stmt, 1, SQL_DIAG_MESSAGE_TEXT, 0, sizeof(buffer),
                  &resLen);
  SQLGetDiagField(SQL_HANDLE_STMT, stmt, 1, SQL_DIAG_MESSAGE_TEXT, buffer, 0, &resLen);
  SQLGetDiagField(SQL_HANDLE_STMT, stmt, 1, SQL_DIAG_MESSAGE_TEXT, buffer, sizeof(buffer),
                  0);
...
BOOST_AUTO_TEST_CASE(TestSQLGetDiagRec) {
  ...
  actualMessage.insert(actualMessage.end(), &message[0], &message[messageLen + 1]);
  ...
  std::vector<SQLWCHAR> reducedExpectedMessage(actualMessage.begin(),
                                               actualMessage.begin() + reducedMessageLen);
  reducedExpectedMessage.push_back(0);
  std::vector<SQLWCHAR> reducedMessage;
  reducedMessage.insert(reducedMessage.end(), &message[0], &message[messageLen + 1]);
  ...
  // There are no checks because we do not really care what is the result of
  // these calls as long as they do not cause segmentation fault.
  SQLGetDiagRec(SQL_HANDLE_STMT, stmt, 1, 0, &nativeError, message, ODBC_BUFFER_SIZE,
                &messageLen);
  SQLGetDiagRec(SQL_HANDLE_STMT, stmt, 1, state, 0, message, ODBC_BUFFER_SIZE,
                &messageLen);
  SQLGetDiagRec(SQL_HANDLE_STMT, stmt, 1, state, &nativeError, 0, ODBC_BUFFER_SIZE,
                &messageLen);
  SQLGetDiagRec(SQL_HANDLE_STMT, stmt, 1, state, &nativeError, message, 0, &messageLen);
  SQLGetDiagRec(SQL_HANDLE_STMT, stmt, 1, state, &nativeError, message, ODBC_BUFFER_SIZE,
                0);
...
BOOST_AUTO_TEST_CASE(TestSQLGetEnvAttr) {
  ...
  // Everything is ok.
  SQLRETURN ret =
      SQLGetEnvAttr(env, SQL_ATTR_ODBC_VERSION, buffer, sizeof(buffer), &resLen);
...
BOOST_AUTO_TEST_CASE(TestSQLError) {
  ...
  ret = SQLError(0, dbc, 0, state, &nativeCode, message, ODBC_BUFFER_SIZE, &messageLen);
  if (ret != SQL_SUCCESS && ret != SQL_NO_DATA) BOOST_FAIL("Unexpected error");
  ...
  ret = SQLError(0, 0, stmt, state, &nativeCode, message, ODBC_BUFFER_SIZE, &messageLen);
  if (ret != SQL_SUCCESS && ret != SQL_NO_DATA) BOOST_FAIL("Unexpected error");
...
BOOST_AUTO_TEST_CASE(TestRowArraySizeGreaterThanOne) {
  ...
  ret = SQLSetStmtAttr(stmt, SQL_ATTR_ROW_ARRAY_SIZE, reinterpret_cast<SQLPOINTER>(val),
                       sizeof(val));
  ...
  BOOST_CHECK_EQUAL("00000001",
                    timestream::odbc::utility::SqlWcharToString(id[0], id_len[0], true));
  BOOST_CHECK_EQUAL("00000002",
                    timestream::odbc::utility::SqlWcharToString(id[1], id_len[1], true));
  BOOST_CHECK_EQUAL("00000003",
                    timestream::odbc::utility::SqlWcharToString(id[2], id_len[2], true));
