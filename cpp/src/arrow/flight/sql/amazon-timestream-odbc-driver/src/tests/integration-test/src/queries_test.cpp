...
struct QueriesTestSuiteFixture : odbc::OdbcTestSuite {
  ...
  void CheckTwoRowsInt(SQLSMALLINT type) {
    ...
    for (SQLSMALLINT i = 0; i < columnsCnt; ++i) {
      ret =
          SQLBindCol(stmt, i + 1, type, &columns[i], sizeof(columns[i]), &columnLens[i]);
...
BOOST_AUTO_TEST_CASE(TestSingleResultUsingBindCol) {
  ...
  ret =
      SQLBindCol(stmt, 6, SQL_C_TYPE_DATE, &fieldDate, sizeof(fieldDate), &fieldDate_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);

  TIME_STRUCT timeValue;
  SQLLEN timeValue_len = 0;
  ret =
      SQLBindCol(stmt, 7, SQL_C_TYPE_TIME, &timeValue, sizeof(timeValue), &timeValue_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  ret = SQLBindCol(stmt, 9, SQL_C_INTERVAL_DAY_TO_SECOND, &daySecond, sizeof(daySecond),
                   &daySecond_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
...
BOOST_AUTO_TEST_CASE(TestNoDataErrorMessage) {
  ...
  std::vector<SQLWCHAR> request =
      MakeSqlBuffer("select * from data_queries_test_db.TestScalarTypes limit 1");
...
BOOST_AUTO_TEST_CASE(TestSingleResultUsingGetData) {
  ...
  SQLBIGINT fieldLong;
  SQLLEN fieldLong_len = 0;
  ret = SQLGetData(stmt, 5, SQL_C_SBIGINT, &fieldLong, sizeof(fieldLong), &fieldLong_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);

  DATE_STRUCT fieldDate{};
  SQLLEN fieldDate_len = 0;
  ret =
      SQLGetData(stmt, 6, SQL_C_TYPE_DATE, &fieldDate, sizeof(fieldDate), &fieldDate_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);

  TIME_STRUCT timeValue;
  SQLLEN timeValue_len = 0;
  ret =
      SQLGetData(stmt, 7, SQL_C_TYPE_TIME, &timeValue, sizeof(timeValue), &timeValue_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS_WITH_INFO, ret);

  SQL_INTERVAL_STRUCT yearMonth;
  SQLLEN yearMonth_len = 0;
  ret = SQLGetData(stmt, 8, SQL_C_INTERVAL_YEAR_TO_MONTH, &yearMonth, sizeof(yearMonth),
                   &yearMonth_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);

  SQL_INTERVAL_STRUCT daySecond;
  SQLLEN daySecond_len = 0;
  ret = SQLGetData(stmt, 9, SQL_C_INTERVAL_DAY_TO_SECOND, &daySecond, sizeof(daySecond),
                   &daySecond_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
...
BOOST_AUTO_TEST_CASE(TestSQLFetchTimeStampAsOtherTypes) {
  ...
  ret = SQLBindCol(stmt, 2, SQL_C_WCHAR, &timestampWchar, sizeof(timestampWchar),
                   &timestampWchar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  SQLLEN fieldDate_len = 0;
  ret =
      SQLBindCol(stmt, 3, SQL_C_TYPE_DATE, &fieldDate, sizeof(fieldDate), &fieldDate_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);

  TIME_STRUCT timeValue;
  SQLLEN timeValue_len = 0;
  ret =
      SQLBindCol(stmt, 4, SQL_C_TYPE_TIME, &timeValue, sizeof(timeValue), &timeValue_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
...
BOOST_AUTO_TEST_CASE(TestSQLFetchTimeAsOtherTypes) {
  ...
  std::vector<SQLWCHAR> request =
      MakeSqlBuffer("select current_time, current_time, current_time, current_time");
  ...
  BOOST_CHECK_EQUAL(utility::SqlCharToString(timestampChar, timestampChar_len),
                    utility::SqlWcharToString(timestampWchar, timestampWchar_len));
...
BOOST_AUTO_TEST_CASE(TestSQLFetchIntervalYearMonthAsOtherTypes) {
  ...
  ret = SQLBindCol(stmt, 2, SQL_C_WCHAR, &yearMonthWchar, sizeof(yearMonthWchar),
                   &yearMonthWchar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  BOOST_CHECK_EQUAL("3-11", utility::SqlCharToString(yearMonthChar, yearMonthChar_len));

  BOOST_CHECK_EQUAL("3-11",
                    utility::SqlWcharToString(yearMonthWchar, yearMonthWchar_len));
}
...
BOOST_AUTO_TEST_CASE(TestSQLFetchIntervalDayMonthAsOtherTypes) {
  ...
  ret = SQLBindCol(stmt, 2, SQL_C_WCHAR, &daySecondWchar, sizeof(daySecondWchar),
                   &daySecondWchar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  BOOST_CHECK_EQUAL("0 04:00:00.000000000",
...
BOOST_AUTO_TEST_CASE(TestTimeSeriesSingleResultUsingBindCol) {
  ...
  ret = SQLBindCol(stmt, 6, SQL_C_WCHAR, timeSeriesWchar, sizeof(timeSeriesWchar),
                   &timeSeriesWchar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  BOOST_CHECK_EQUAL(expected,
                    utility::SqlCharToString(timeSeriesChar, timeSeriesChar_len));

  BOOST_CHECK_EQUAL(
      expected, utility::SqlWcharToString(timeSeriesWchar, timeSeriesWchar_len, true));
}
...
BOOST_AUTO_TEST_CASE(TestArraySingleResultUsingBindCol) {
  ...
  // fetch result as a string
  ret = SQLBindCol(stmt, 1, SQL_C_CHAR, arrayChar1, sizeof(arrayChar1), &arrayChar1_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  // fetch result as a string
  ret = SQLBindCol(stmt, 3, SQL_C_CHAR, arrayChar2, sizeof(arrayChar2), &arrayChar2_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  BOOST_CHECK_EQUAL("[1,2,3]", utility::SqlCharToString(arrayChar1, arrayChar1_len));
  BOOST_CHECK_EQUAL("[1,2,3]",
                    utility::SqlWcharToString(arrayWchar1, arrayWchar1_len, true));
  BOOST_CHECK_EQUAL("-", utility::SqlCharToString(arrayChar2, arrayChar2_len));
  BOOST_CHECK_EQUAL("-", utility::SqlWcharToString(arrayWchar2, arrayWchar2_len, true));
}
...
BOOST_AUTO_TEST_CASE(TestRowSingleResultUsingBindCol) {
  ...
  // fetch result as a unicode string
  ret = SQLBindCol(stmt, 2, SQL_C_WCHAR, rowWchar, sizeof(rowWchar), &rowWchar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  BOOST_CHECK_EQUAL("(1,2,3)", utility::SqlWcharToString(rowWchar, rowWchar_len, true));
}
...
BOOST_AUTO_TEST_CASE(TestNullSingleResultUsingBindCol) {
  ...
  // fetch result as a string
  ret = SQLBindCol(stmt, 1, SQL_C_CHAR, nullChar, sizeof(nullChar), &nullChar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  // fetch result as a unicode string
  ret = SQLBindCol(stmt, 2, SQL_C_WCHAR, nullWchar, sizeof(nullWchar), &nullWchar_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
  ...
  BOOST_CHECK_EQUAL("-", utility::SqlWcharToString(nullWchar, nullWchar_len, true));
}
...
BOOST_AUTO_TEST_CASE(TestSQLFetchBigTablePagination) {
  if (CheckEnvVarSetToTrue("BIG_TABLE_PAGINATION_TEST_ENABLE")) {
    ...
    SQLLEN fieldLong_len = 0;
    ret =
        SQLBindCol(stmt, 2, SQL_C_SBIGINT, &fieldLong, sizeof(fieldLong), &fieldLong_len);
    BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
    double fieldDouble = 0;
    SQLLEN fieldDouble_len = 0;
    ...
    std::cout << boost::unit_test::framework::current_test_case().p_name << " is skipped"
              << std::endl;
  }
}
...
BOOST_AUTO_TEST_CASE(TestSQLFetchPaginationEmptyTable) {
  ...
  SQLRETURN ret;
  std::vector<SQLWCHAR> request =
      MakeSqlBuffer("select measure_name, time from data_queries_test_db.EmptyTable");

  ret = SQLExecDirect(stmt, request.data(), SQL_NTS);
  BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS_WITH_INFO);
  BOOST_REQUIRE_NE(
      GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt).find("01000: Query result is empty"),
      std::string::npos);
...
BOOST_AUTO_TEST_CASE(TestSQLRowCountWithNoResults) {
  ...
  std::vector<SQLWCHAR> sql =
      MakeSqlBuffer("select * from data_queries_test_db.TestScalarTypes limit 20");
...
BOOST_AUTO_TEST_CASE(TestTwoRowsInt8) { CheckTwoRowsInt<signed char>(SQL_C_STINYINT); }

BOOST_AUTO_TEST_CASE(TestTwoRowsUint8) { CheckTwoRowsInt<unsigned char>(SQL_C_UTINYINT); }

BOOST_AUTO_TEST_CASE(TestTwoRowsInt16) { CheckTwoRowsInt<signed short>(SQL_C_SSHORT); }

BOOST_AUTO_TEST_CASE(TestTwoRowsUint16) { CheckTwoRowsInt<unsigned short>(SQL_C_USHORT); }

BOOST_AUTO_TEST_CASE(TestTwoRowsInt32) { CheckTwoRowsInt<SQLINTEGER>(SQL_C_SLONG); }

BOOST_AUTO_TEST_CASE(TestTwoRowsUint32) { CheckTwoRowsInt<SQLUINTEGER>(SQL_C_ULONG); }

BOOST_AUTO_TEST_CASE(TestTwoRowsInt64) { CheckTwoRowsInt<int64_t>(SQL_C_SBIGINT); }

BOOST_AUTO_TEST_CASE(TestTwoRowsUint64) { CheckTwoRowsInt<uint64_t>(SQL_C_UBIGINT); }
...
BOOST_AUTO_TEST_CASE(TestDefaultValues) {
  ...
  // Binding columns.
  ret = SQLBindCol(stmt, 1, SQL_C_CHAR, &strColumn, ODBC_BUFFER_SIZE, &columnLens[0]);
  if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
  ...
  for (SQLSMALLINT i = 0; i < 3; ++i) BOOST_CHECK_NE(columnLens[i], SQL_NULL_DATA);
...
BOOST_AUTO_TEST_CASE(TestSQLMoreResults) {
  ...
  SQLRETURN ret;
  std::vector<SQLWCHAR> request =
      MakeSqlBuffer("select * from data_queries_test_db.TestScalarTypes limit 4");
...
BOOST_AUTO_TEST_CASE(TestExecuteAfterCursorClose) {
  ...
  // Binding columns.
  SQLRETURN ret =
      SQLBindCol(stmt, 1, SQL_C_WCHAR, &strField, sizeof(strField), &strFieldLen);
  ...
  BOOST_CHECK_EQUAL(utility::SqlWcharToString(strField, strFieldLen, true), "00000005");
...
BOOST_AUTO_TEST_CASE(TestSingleResultUsingGetDataWideChar) {
  ...
  BOOST_CHECK_EQUAL(u8"美西-5",
                    utility::SqlWcharToString(fieldString, fieldString_len, true));
...
BOOST_AUTO_TEST_CASE(TestSingleResultSelectWideCharUsingGetDataNarrowChar) {
  ...
  ret =
      SQLGetData(stmt, 2, SQL_C_CHAR, fieldString, sizeof(fieldString), &fieldString_len);
  BOOST_CHECK_EQUAL(SQL_SUCCESS, ret);
