...
struct MetaQueriesTestSuiteFixture : public odbc::OdbcTestSuite {
  ...
  void CheckSingleRowResultSetWithGetData(
      SQLHSTMT stmt, SQLUSMALLINT columnIndex = 1, const std::string expectedValue = "",
    ...
    for (int i = 1; i <= columnIndex; i++) {
      ...
      if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
    ...
    BOOST_CHECK_EQUAL(GetOdbcErrorState(SQL_HANDLE_STMT, stmt), expectedErrorState);
  }
  ...
  bool WasNull(const SQLLEN length) const { return (length == SQL_NULL_DATA); }

  void SQLColumnsBindColumns(
      SQLHSTMT stmt, char table_cat[], SQLLEN& table_cat_len, char table_schem[],
      SQLLEN& table_schem_len, char table_name[], SQLLEN& table_name_len,
      char column_name[], SQLLEN& column_name_len, SQLSMALLINT& data_type,
      SQLLEN& data_type_len, char type_name[], SQLLEN& type_name_len,
      SQLINTEGER& column_size, SQLLEN& column_size_len, SQLINTEGER& buffer_length,
      SQLLEN& buffer_length_len, SQLSMALLINT& decimal_digits, SQLLEN& decimal_digits_len,
      SQLSMALLINT& num_prec_radix, SQLLEN& num_prec_radix_len, SQLSMALLINT& nullable,
      SQLLEN& nullable_len, char remarks[], SQLLEN& remarks_len, char column_def[],
      SQLLEN& column_def_len, SQLSMALLINT& sql_data_type, SQLLEN& sql_data_type_len,
      SQLSMALLINT& sql_datetime_sub, SQLLEN& sql_datetime_sub_len,
      SQLINTEGER& char_octet_length, SQLLEN& char_octet_length_len,
      SQLINTEGER& ordinal_position, SQLLEN& ordinal_position_len, char is_nullable[],
      SQLLEN& is_nullable_len) {
    SQLRETURN ret = SQL_SUCCESS;
    ret = SQLBindCol(stmt, 1, SQL_C_CHAR, table_cat, C_STR_LEN_DEFAULT, &table_cat_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret =
        SQLBindCol(stmt, 2, SQL_C_CHAR, table_schem, C_STR_LEN_DEFAULT, &table_schem_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 3, SQL_C_CHAR, table_name, C_STR_LEN_DEFAULT, &table_name_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret =
        SQLBindCol(stmt, 4, SQL_C_CHAR, column_name, C_STR_LEN_DEFAULT, &column_name_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret =
        SQLBindCol(stmt, 5, SQL_SMALLINT, &data_type, sizeof(data_type), &data_type_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 6, SQL_C_CHAR, type_name, C_STR_LEN_DEFAULT, &type_name_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ...
    ret = SQLBindCol(stmt, 8, SQL_INTEGER, &buffer_length, sizeof(buffer_length),
                     &buffer_length_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 9, SQL_SMALLINT, &decimal_digits, sizeof(decimal_digits),
                     &decimal_digits_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 10, SQL_SMALLINT, &num_prec_radix, sizeof(num_prec_radix),
                     &num_prec_radix_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 11, SQL_SMALLINT, &nullable, sizeof(nullable), &nullable_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 12, SQL_C_CHAR, remarks, C_STR_LEN_DEFAULT, &remarks_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret =
        SQLBindCol(stmt, 13, SQL_C_CHAR, column_def, C_STR_LEN_DEFAULT, &column_def_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 14, SQL_SMALLINT, &sql_data_type, sizeof(sql_data_type),
                     &sql_data_type_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 15, SQL_SMALLINT, &sql_datetime_sub, sizeof(sql_datetime_sub),
                     &sql_datetime_sub_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 16, SQL_INTEGER, &char_octet_length, sizeof(char_octet_length),
                     &char_octet_length_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
    ret = SQLBindCol(stmt, 17, SQL_INTEGER, &ordinal_position, sizeof(ordinal_position),
                     &ordinal_position_len);
    BOOST_CHECK(SQL_SUCCEEDED(ret));
   ...
   */
  void callSQLColAttribute(SQLHSTMT stmt, const SQLCHAR* query, SQLSMALLINT fieldId,
    ...
    ret = SQLColAttribute(stmt, 1, fieldId, strBuf, sizeof(strBuf), &resLen, nullptr);
    if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
   ...
   * @param expectedVal Expected string data.
   */
  void callSQLColAttributes(SQLHSTMT stmt, const SQLCHAR* query, SQLSMALLINT fieldId,
                            const std::string& expectedVal) {
    SQLWCHAR strBuf[1024] = {0};
    std::vector<SQLWCHAR> wQuery = MakeSqlBuffer(reinterpret_cast<const char*>(query));
    ...
    // non-empty string. This behavior is out of the driver's control.
    SQLSMALLINT resLen = 0;
    ret = SQLColAttributes(stmt, 1, fieldId, strBuf, sizeof(strBuf), &resLen, nullptr);
    if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
  ...
  void CheckColumnMetaWithSQLDescribeCol(SQLHSTMT stmt, SQLUSMALLINT idx,
                                         ...
                                         SQLSMALLINT expDataType, SQLULEN expSize,
                                         SQLSMALLINT expScale,
    ...
    SQLRETURN ret = SQLDescribeCol(stmt, idx, &name[0], (SQLSMALLINT)name.size(),
  ...
  void CheckSQLDescribeColPrecisionAndScale(F func) {
    ...
    CheckColumnMetaWithSQLDescribeCol(stmt, 2, "time", SQL_TYPE_TIMESTAMP, 19, -1,
                                      SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLDescribeCol(stmt, 3, "flag", SQL_BIT, 1, -1,
                                      SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLDescribeCol(stmt, 4, "rebuffering_ratio", SQL_DOUBLE, 15, 15,
                                      SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLDescribeCol(stmt, 5, "video_startup_time", SQL_BIGINT, 19, 0,
                                      SQL_NULLABLE_UNKNOWN);
  }
  ...
  void CheckColumnMetaWithSQLColAttribute(SQLHSTMT stmt, SQLUSMALLINT idx,
                                          const std::string& expName, SQLLEN expDataType,
                                          SQLULEN expSize, SQLLEN expScale,
    ...
    SQLRETURN ret =
        SQLColAttribute(stmt, idx, SQL_DESC_NAME, &name[0],
                        (SQLSMALLINT)name.size() * sizeof(SQLWCHAR), &nameLen, nullptr);
    ODBC_FAIL_ON_ERROR(ret, SQL_HANDLE_STMT, stmt);
  ...
  void CheckSQLColAttributePrecisionAndScale(F func) {
    ...
    CheckColumnMetaWithSQLColAttribute(stmt, 1, "device_id", SQL_VARCHAR,
                                       TIMESTREAM_SQL_MAX_LENGTH, -1,
                                       SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLColAttribute(stmt, 2, "time", SQL_TYPE_TIMESTAMP, 19, -1,
                                       SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLColAttribute(stmt, 3, "flag", SQL_BIT, 1, -1,
                                       SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLColAttribute(stmt, 4, "rebuffering_ratio", SQL_DOUBLE, 15, 15,
                                       SQL_NULLABLE_UNKNOWN);
    CheckColumnMetaWithSQLColAttribute(stmt, 5, "video_startup_time", SQL_BIGINT, 19, 0,
                                       SQL_NULLABLE_UNKNOWN);
  }
  ...
  void CheckSQLGetTypeInfoResult(const std::string& exptectedTypeName,
    ...
    SQLLEN dataTypeLen;
    ret = SQLGetData(stmt, 2, SQL_SMALLINT, &dataType, sizeof(dataType), &dataTypeLen);
...
BOOST_AUTO_TEST_CASE(TestSQLDescribeColGetTypeInfo) {
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 1, "TYPE_NAME", SQL_VARCHAR,
                                    TIMESTREAM_SQL_MAX_LENGTH, -1, SQL_NO_NULLS);
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 8, "CASE_SENSITIVE", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 9, "SEARCHABLE", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 10, "UNSIGNED_ATTRIBUTE", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 11, "FIXED_PREC_SCALE", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 12, "AUTO_UNIQUE_VALUE", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 13, "LOCAL_TYPE_NAME", SQL_VARCHAR,
                                    TIMESTREAM_SQL_MAX_LENGTH, -1, SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 14, "MINIMUM_SCALE", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 15, "MAXIMUM_SCALE", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 16, "SQL_DATA_TYPE", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 17, "SQL_DATETIME_SUB", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 18, "NUM_PREC_RADIX", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 19, "INTERVAL_PRECISION", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);
}
...
BOOST_AUTO_TEST_CASE(TestGetTypeInfoAllTypes) {
  ...
  CheckSQLGetTypeInfoResult("INTERVAL_DAY_TO_SECOND", SQL_INTERVAL_DAY_TO_SECOND);
  CheckSQLGetTypeInfoResult("INTERVAL_YEAR_TO_MONTH", SQL_INTERVAL_YEAR_TO_MONTH);
...
BOOST_AUTO_TEST_CASE(TestDateTypeColumnAttributeField) {
  ...
  std::vector<SQLWCHAR> req =
      MakeSqlBuffer("select date(time) from meta_queries_test_db.TestColumnsMetadata2");
  SQLExecDirect(stmt, req.data(), SQL_NTS);
...
BOOST_AUTO_TEST_CASE(TestColAttributesColumnLength) {
  ...
  SQLRETURN ret = SQLColAttribute(stmt, 1, SQL_COLUMN_LENGTH, strBuf, sizeof(strBuf),
                                  &strLen, &intVal);
...
BOOST_AUTO_TEST_CASE(TestColAttributesColumnPresicion) {
  ...
  SQLRETURN ret = SQLColAttribute(stmt, 1, SQL_COLUMN_PRECISION, strBuf, sizeof(strBuf),
                                  &strLen, &intVal);
...
BOOST_AUTO_TEST_CASE(TestColAttributeWithOneTable) {
  ...
  for (int i = 1; i <= numTests; i++) {
    ret = SQLColAttribute(stmt, SQLUSMALLINT(i), SQL_DESC_TYPE, nullptr, 0, nullptr,
                          &intVal);
    ...
    ret = SQLColAttribute(stmt, SQLUSMALLINT(i), SQL_DESC_NAME, strBuf, sizeof(strBuf),
                          &strLen, &intVal);
...
BOOST_AUTO_TEST_CASE(TestColAttributeDataTypesAndColumnNames) {
  ...
  for (int i = 1; i <= numTests; i++) {
    SQLRETURN ret = SQLColAttribute(stmt, SQLUSMALLINT(i), SQL_DESC_TYPE, nullptr, 0,
                                    nullptr, &intVal);
    if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescBaseColumnName) {
  ...
  callSQLColAttribute(stmt, req, SQL_DESC_BASE_COLUMN_NAME, std::string("fuel-reading"));
}
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescDisplaySize) {
  ...
  callSQLColAttribute(stmt, req1, SQL_DESC_DISPLAY_SIZE, TIMESTREAM_SQL_MAX_LENGTH);
  ...
  const SQLCHAR req6[] = "select flag from meta_queries_test_db.TestColumnsMetadata1";
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescLocalTypeName) {
  ...
  callSQLColAttribute(stmt, req1, SQL_DESC_LOCAL_TYPE_NAME, SqlTypeName::DOUBLE);

  const SQLCHAR req2[] =
      "select device_id from meta_queries_test_db.TestColumnsMetadata1";

  // SQL_VARCHAR should have type name SqlTypeName::VARCHAR
  callSQLColAttribute(stmt, req2, SQL_DESC_LOCAL_TYPE_NAME, SqlTypeName::VARCHAR);
  ...
  const SQLCHAR req4[] = "select time from meta_queries_test_db.TestColumnsMetadata1";

  // SQL_TYPE_TIMESTAMP should have type name SqlTypeName::TIMESTAMP
  callSQLColAttribute(stmt, req4, SQL_DESC_LOCAL_TYPE_NAME, SqlTypeName::TIMESTAMP);
  ...
  callSQLColAttribute(stmt, req5, SQL_DESC_LOCAL_TYPE_NAME, SqlTypeName::BIGINT);
}
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescNullable) {
  ...
  const SQLCHAR req2[] = "select flag from meta_queries_test_db.TestColumnsMetadata1";
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescSearchable) {
  ...
  const SQLCHAR req[] = "select device_id from meta_queries_test_db.TestColumnsMetadata1";
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescType) {
  ...
  const SQLCHAR req3[] = "select time from meta_queries_test_db.TestColumnsMetadata1";
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescUnnamed) {
  ...
  const SQLCHAR req[] = "select time from meta_queries_test_db.TestColumnsMetadata1";
...
BOOST_AUTO_TEST_CASE(TestColAttributeDescUpdatable) {
  ...
  callSQLColAttribute(stmt, req, SQL_DESC_UPDATABLE, SQL_ATTR_READWRITE_UNKNOWN);
}
...
BOOST_AUTO_TEST_CASE(TestColAttributesColumnScale) {
  ...
  SQLRETURN ret = SQLColAttribute(stmt, 1, SQL_COLUMN_SCALE, strBuf, sizeof(strBuf),
                                  &strLen, &intVal);
...
BOOST_AUTO_TEST_CASE(TestColAttributesColumnLengthPrepare) {
  ...
  ret = SQLColAttribute(stmt, 1, SQL_COLUMN_LENGTH, strBuf, sizeof(strBuf), &strLen,
                        &intVal);
...
BOOST_AUTO_TEST_CASE(TestColAttributesColumnPresicionPrepare) {
  ...
  ret = SQLColAttribute(stmt, 1, SQL_COLUMN_PRECISION, strBuf, sizeof(strBuf), &strLen,
                        &intVal);
...
BOOST_AUTO_TEST_CASE(TestColAttributesColumnScalePrepare) {
  ...
  ret = SQLColAttribute(stmt, 1, SQL_COLUMN_SCALE, strBuf, sizeof(strBuf), &strLen,
                        &intVal);
...
BOOST_AUTO_TEST_CASE(TestSQLDescribeColSQLColumns) {
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 8, "BUFFER_LENGTH", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 9, "DECIMAL_DIGITS", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 10, "NUM_PREC_RADIX", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 14, "SQL_DATA_TYPE", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 15, "SQL_DATETIME_SUB", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 16, "CHAR_OCTET_LENGTH", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 17, "ORDINAL_POSITION", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsDataTypes) {
  ...
  ret =
      SQLBindCol(stmt, 4, SQL_C_CHAR, column_name, sizeof(column_name), &column_name_len);
  BOOST_CHECK(SQL_SUCCEEDED(ret));
  ret = SQLBindCol(stmt, 5, SQL_SMALLINT, &data_type, sizeof(data_type), &data_type_len);
  BOOST_CHECK(SQL_SUCCEEDED(ret));
  ret = SQLBindCol(stmt, 6, SQL_C_CHAR, type_name, sizeof(type_name), &type_name_len);
  BOOST_CHECK(SQL_SUCCEEDED(ret));
  ret = SQLBindCol(stmt, 11, SQL_SMALLINT, &nullable, sizeof(nullable), &nullable_len);
  BOOST_CHECK(SQL_SUCCEEDED(ret));
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsTableNameOnly) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ...
    ret = SQLColumns(stmt, all.data(), SQL_NTS, nullptr, 0, table.data(), SQL_NTS,
                     all.data(), SQL_NTS);
  } else {
    databaseColumnIndex = 1;
    ret = SQLColumns(stmt, nullptr, 0, all.data(), SQL_NTS, table.data(), SQL_NTS,
                     all.data(), SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsNull) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLColumns(stmt, empty.data(), SQL_NTS, nullptr, 0, table.data(), SQL_NTS,
                     column.data(), SQL_NTS);
  } else {
    ret = SQLColumns(stmt, nullptr, 0, empty.data(), SQL_NTS, table.data(), SQL_NTS,
                     column.data(), SQL_NTS);
  }
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLColumns(stmt, empty.data(), SQL_NTS, nullptr, 0, nullptr, 0, column.data(),
                     SQL_NTS);
  } else {
    ret = SQLColumns(stmt, nullptr, 0, empty.data(), SQL_NTS, nullptr, 0, column.data(),
                     SQL_NTS);
  }
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLColumns(stmt, empty.data(), SQL_NTS, nullptr, 0, nullptr, 0, nullptr, 0);
  } else {
    ret = SQLColumns(stmt, nullptr, 0, empty.data(), SQL_NTS, nullptr, 0, nullptr, 0);
  }
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsEmptyMetadataIdTrue) {
  ...
  ret = SQLColumns(stmt, empty.data(), SQL_NTS, empty.data(), SQL_NTS, table.data(),
  ...
  if (DATABASE_AS_SCHEMA) {
    ...
    ret = SQLColumns(stmt, empty.data(), SQL_NTS, database.data(), SQL_NTS, table.data(),
                     SQL_NTS, column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS);

    // schemaName is empty case
    ret = SQLColumns(stmt, any.data(), SQL_NTS, empty.data(), SQL_NTS, table.data(),
                     SQL_NTS, column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS_WITH_INFO);
    CheckSQLStatementDiagnosticError("01000");
    ...
    ret = SQLColumns(stmt, any.data(), SQL_NTS, database.data(), SQL_NTS, empty.data(),
                     SQL_NTS, column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS_WITH_INFO);
    CheckSQLStatementDiagnosticError("01000");
    BOOST_REQUIRE_EQUAL("01000: Schema and table name should not be empty.",
    ...
    ret = SQLColumns(stmt, any.data(), SQL_NTS, database.data(), SQL_NTS, table.data(),
    ...
    // catalogName is empty case
    ret = SQLColumns(stmt, empty.data(), SQL_NTS, any.data(), SQL_NTS, table.data(),
                     SQL_NTS, column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS_WITH_INFO);
    CheckSQLStatementDiagnosticError("01000");
    ...
    // schemaName is empty case
    ret = SQLColumns(stmt, database.data(), SQL_NTS, empty.data(), SQL_NTS, table.data(),
                     SQL_NTS, column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS);

    // tableName is empty case
    ret = SQLColumns(stmt, database.data(), SQL_NTS, any.data(), SQL_NTS, empty.data(),
                     SQL_NTS, column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS_WITH_INFO);
    CheckSQLStatementDiagnosticError("01000");
    BOOST_REQUIRE_EQUAL("01000: Catalog and table name should not be empty.",
    ...
    // columnName is empty case
    ret = SQLColumns(stmt, database.data(), SQL_NTS, any.data(), SQL_NTS, table.data(),
                     SQL_NTS, empty.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS_WITH_INFO);
    CheckSQLStatementDiagnosticError("01000");
    BOOST_REQUIRE_EQUAL(
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsNullMetadataIdTrue) {
  ...
  ret = SQLColumns(stmt, nullptr, 0, nullptr, 0, table.data(), SQL_NTS, column.data(),
                   SQL_NTS);
  BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
  ...
  if (DATABASE_AS_SCHEMA) {
    ...
    ret = SQLColumns(stmt, nullptr, 0, database.data(), SQL_NTS, table.data(), SQL_NTS,
                     column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS);

    // schemaName is null case
    ret = SQLColumns(stmt, any.data(), SQL_NTS, nullptr, 0, table.data(), SQL_NTS,
                     column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
    CheckSQLStatementDiagnosticError("HY009");
    BOOST_REQUIRE_EQUAL(
    ...
    ret = SQLColumns(stmt, any.data(), SQL_NTS, database.data(), SQL_NTS, nullptr, 0,
                     column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
    CheckSQLStatementDiagnosticError("HY009");
    BOOST_REQUIRE_EQUAL(
    ...
    // catalogName is null case
    ret = SQLColumns(stmt, nullptr, 0, any.data(), SQL_NTS, table.data(), SQL_NTS,
                     column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
    CheckSQLStatementDiagnosticError("HY009");
    BOOST_REQUIRE_EQUAL(
    ...
    // schemaName is null case
    ret = SQLColumns(stmt, database.data(), SQL_NTS, nullptr, 0, table.data(), SQL_NTS,
                     column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS);

    // tableName is null case
    ret = SQLColumns(stmt, database.data(), SQL_NTS, any.data(), SQL_NTS, nullptr, 0,
                     column.data(), SQL_NTS);
    BOOST_REQUIRE_EQUAL(ret, SQL_ERROR);
    CheckSQLStatementDiagnosticError("HY009");
    BOOST_REQUIRE_EQUAL(
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsNullMetadataIdFalse) {
  ...
  // catalogName and schemaName are null case
  SQLRETURN ret = SQLColumns(stmt, nullptr, 0, nullptr, 0, table.data(), SQL_NTS,
                             column.data(), SQL_NTS);
  BOOST_REQUIRE_EQUAL(ret, SQL_SUCCESS);
...
BOOST_AUTO_TEST_CASE(TestGetColumnsWithUnsupportedDatabase) {
  ...
  CheckSQLStatementDiagnosticError("01000");
  if (DATABASE_AS_SCHEMA) {
    BOOST_REQUIRE_EQUAL("01000: Empty result set is returned as catalog is set to \"" +
                            SqlWcharToString(database.data()) +
                            "\" and Timestream does not have catalogs",
                        GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
  } else {
    BOOST_REQUIRE_EQUAL("01000: Empty result set is returned as schema is set to \"" +
                            SqlWcharToString(database.data()) +
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsEmpty) {
  ...
  // table is empty case
  ret = SQLColumns(stmt, empty.data(), SQL_NTS, any.data(), SQL_NTS, empty.data(),
                   SQL_NTS, column.data(), SQL_NTS);
...
BOOST_DATA_TEST_CASE_F(MetaQueriesTestSuiteFixture, TestGetDataWithColumnsUnicode,
  ...
  BOOST_CHECK_EQUAL(
      "地区", utility::SqlWcharToString(column_name, column_name_len));  // COLUMN_NAME
  BOOST_CHECK_EQUAL(SQL_VARCHAR, data_type);                             // DATA_TYPE
}
...
BOOST_AUTO_TEST_CASE(TestGetDataWithColumnsNonExist) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLColumns(stmt, empty.data(), SQL_NTS, nullptr, 0, correctTable.data(),
                     SQL_NTS, correctColumn.data(), SQL_NTS);
  } else {
    ret = SQLColumns(stmt, nullptr, 0, empty.data(), SQL_NTS, correctTable.data(),
                     SQL_NTS, correctColumn.data(), SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesSearchPatternReturnsOne) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, empty.data(), SQL_NTS, nullptr, 0, testTablePattern.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
  } else {
    ret = SQLTables(stmt, nullptr, 0, empty.data(), SQL_NTS, testTablePattern.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
  }
  ...
  CheckSingleRowResultSetWithGetData(stmt, 3,
                                     utility::SqlWcharToString(testTable1.data()));
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, empty.data(), SQL_NTS, databasePattern.data(), SQL_NTS,
                    testTablePattern.data(), SQL_NTS, empty.data(), SQL_NTS);
  } else {
    ret = SQLTables(stmt, databasePattern.data(), SQL_NTS, empty.data(), SQL_NTS,
                    testTablePattern.data(), SQL_NTS, empty.data(), SQL_NTS);
  }
  ...
  CheckSingleRowResultSetWithGetData(stmt, 3,
                                     utility::SqlWcharToString(testTable2.data()));
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, empty.data(), SQL_NTS, testDatabase.data(), SQL_NTS, nullptr, 0,
                    empty.data(), SQL_NTS);
    if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));

    CheckSingleRowResultSetWithGetData(stmt, 2,
                                       utility::SqlWcharToString(testDatabase.data()));

  } else {
    ret = SQLTables(stmt, testDatabase.data(), SQL_NTS, empty.data(), SQL_NTS, nullptr, 0,
                    empty.data(), SQL_NTS);
    if (!SQL_SUCCEEDED(ret)) BOOST_FAIL(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));

    CheckSingleRowResultSetWithGetData(stmt, 1,
                                       utility::SqlWcharToString(testDatabase.data()));
  }
}
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesSearchPatternReturnsMany) {
  ...
  if (tableMatches < expectedTableMatches) {
    std::string sqlMessage = "Expected to find " + std::to_string(expectedTableMatches) +
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesIdentifierReturnsNone) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, nullptr, 0, searchPattern.data(), SQL_NTS, searchPattern.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
  } else {
    ret = SQLTables(stmt, searchPattern.data(), SQL_NTS, nullptr, 0, searchPattern.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesIdentifierReturnsOne) {
  ...
  std::vector<SQLWCHAR> empty = {0};
  std::vector<SQLWCHAR> testDatabaseIdentifier = MakeSqlBuffer("meTa_QueRiEs_Test_Db");
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, empty.data(), SQL_NTS, testDatabaseIdentifier.data(), SQL_NTS,
                    testTableIdentifier.data(), SQL_NTS, empty.data(), SQL_NTS);
  } else {
    ret = SQLTables(stmt, testDatabaseIdentifier.data(), SQL_NTS, empty.data(), SQL_NTS,
                    testTableIdentifier.data(), SQL_NTS, empty.data(), SQL_NTS);
  }
  ...
  CheckSingleRowResultSetWithGetData(stmt, 3,
                                     utility::SqlWcharToString(testTable.data()));
}
...
BOOST_AUTO_TEST_CASE(TestGetTablesPassNullTableMetadataIdTrue) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ExpectSQLTablesReject(
        empty.data(), SQL_NTS, testDatabase.data(), SQL_NTS, nullptr, 0, empty.data(),
        SQL_NTS, "HY009",
    ...
    ExpectSQLTablesReject(
        testDatabase.data(), SQL_NTS, empty.data(), SQL_NTS, nullptr, 0, empty.data(),
        SQL_NTS, "HY009",
        "The SQL_ATTR_METADATA_ID statement attribute is set to SQL_TRUE, "
        "and CatalogName or "
        "the TableName argument was a null pointer.");
...
BOOST_AUTO_TEST_CASE(TestGetTablesPassNullDatabaseMetadataIdTrue) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ExpectSQLTablesReject(
        empty.data(), SQL_NTS, nullptr, 0, testTable.data(), SQL_NTS, empty.data(),
        SQL_NTS, "HY009",
        "The SQL_ATTR_METADATA_ID statement attribute is set to SQL_TRUE, "
        "and SchemaName or "
        "the TableName argument was a null pointer.");
    ...
    ExpectSQLTablesReject(
        nullptr, 0, empty.data(), SQL_NTS, testTable.data(), SQL_NTS, empty.data(),
        SQL_NTS, "HY009",
        "The SQL_ATTR_METADATA_ID statement attribute is set to SQL_TRUE, "
        "and CatalogName or "
        "the TableName argument was a null pointer.");
...
BOOST_AUTO_TEST_CASE(TestGetTablesPassNullToUnsupportedMetadataIdTrue) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ...
    ret = SQLTables(stmt, nullptr, 0, testDatabase.data(), SQL_NTS, testTable.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
  } else {
    // only catalogs supported, driver should ignore schemaName being nullptr
    ret = SQLTables(stmt, testDatabase.data(), SQL_NTS, nullptr, 0, testTable.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesReturnsOneWithTableTypes) {
  ...
  std::vector<SQLWCHAR> tableTypes =
      MakeSqlBuffer("TABLE,VIEW");  // Test that VIEW type is ignored by ODBC driver
  SQLRETURN ret;

  ConnectToTS();

  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, empty.data(), SQL_NTS, nullptr, 0, table.data(), SQL_NTS,
                    tableTypes.data(), SQL_NTS);
  } else {
    ret = SQLTables(stmt, nullptr, 0, empty.data(), SQL_NTS, table.data(), SQL_NTS,
                    tableTypes.data(), SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestGetDatabasesWithSQLTables) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ...
    ret = SQLTables(stmt, empty.data(), SQL_NTS, schemas.data(), SQL_NTS, empty.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
    ...
    ret = SQLTables(stmt, catalogs.data(), SQL_NTS, empty.data(), SQL_NTS, empty.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
...
// TableType argument
BOOST_DATA_TEST_CASE_F(MetaQueriesTestSuiteFixture, TestGetTableTypesWithSQLTables,
                       data::make({false, true}), useIdentifier) {
  ...
  ret = SQLTables(stmt, empty.data(), SQL_NTS, empty.data(), SQL_NTS, empty.data(),
                  SQL_NTS, tableType.data(), SQL_NTS);
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesReturnsNone) {
  ...
  SQLRETURN ret = SQLTables(stmt, empty.data(), SQL_NTS, nullptr, 0, table.data(),
                            SQL_NTS, empty.data(), SQL_NTS);
  ...
  ret = SQLTables(stmt, empty.data(), SQL_NTS, empty.data(), SQL_NTS, correctTable.data(),
                  SQL_NTS, empty.data(), SQL_NTS);
  ...
  if (DATABASE_AS_SCHEMA) {
    ...
    ret = SQLTables(stmt, catalog.data(), SQL_NTS, empty.data(), SQL_NTS, empty.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
    ...
    ret = SQLTables(stmt, empty.data(), SQL_NTS, schema.data(), SQL_NTS, empty.data(),
                    SQL_NTS, empty.data(), SQL_NTS);
...
BOOST_AUTO_TEST_CASE(TestGetDataWithTablesReturnsMany) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLTables(stmt, empty.data(), SQL_NTS, nullptr, 0, nullptr, 0, empty.data(),
                    SQL_NTS);
  } else {
    ret = SQLTables(stmt, nullptr, 0, empty.data(), SQL_NTS, nullptr, 0, empty.data(),
                    SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestSQLColumnWithSQLBindCols) {
  ...
  SQLColumnsBindColumns(
      ...
      table_name_len, column_name, column_name_len, data_type, data_type_len, type_name,
      type_name_len, column_size, column_size_len, buffer_length, buffer_length_len,
      decimal_digits, decimal_digits_len, num_prec_radix, num_prec_radix_len, nullable,
      nullable_len, remarks, remarks_len, column_def, column_def_len, sql_data_type,
      sql_data_type_len, sql_datetime_sub, sql_datetime_sub_len, char_octet_length,
      char_octet_length_len, ordinal_position, ordinal_position_len, is_nullable,
      is_nullable_len);
  ...
  ret = SQLColAttribute(stmt, 2, SQL_DESC_NAME, attrColumnName, sizeof(attrColumnName),
                        &attrColumnNameLen,
                        nullptr);  // COLUMN_NAME, NOT NULL
  if (!SQL_SUCCEEDED(ret)) {
    BOOST_ERROR(GetOdbcErrorMessage(SQL_HANDLE_STMT, stmt));
  }
  BOOST_CHECK_EQUAL("TABLE_SCHEM",
                    utility::SqlWcharToString(attrColumnName, attrColumnNameLen));
...
BOOST_AUTO_TEST_CASE(TestSQLForeignKeys) {
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 9, "KEY_SEQ", SQL_INTEGER, 10, 0, SQL_NO_NULLS);
...
BOOST_AUTO_TEST_CASE(TestSQLPrimaryKeys) {
  ...
  SQLRETURN ret = SQLPrimaryKeys(stmt, nullptr, 0, nullptr, 0, table.data(), SQL_NTS);
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 5, "KEY_SEQ", SQL_INTEGER, 10, 0, SQL_NO_NULLS);
...
BOOST_AUTO_TEST_CASE(TestSQLSpecialColumns) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret =
        SQLSpecialColumns(stmt, SQL_BEST_ROWID, empty.data(), SQL_NTS, database.data(),
                          SQL_NTS, table.data(), SQL_NTS, SQL_SCOPE_CURROW, SQL_NO_NULLS);
  } else {
    ret =
        SQLSpecialColumns(stmt, SQL_BEST_ROWID, database.data(), SQL_NTS, empty.data(),
                          SQL_NTS, table.data(), SQL_NTS, SQL_SCOPE_CURROW, SQL_NO_NULLS);
  }
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 1, "SCOPE", SQL_INTEGER, 10, 0, SQL_NULLABLE);
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 6, "BUFFER_LENGTH", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 7, "DECIMAL_DIGITS", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);
...
BOOST_AUTO_TEST_CASE(TestSQLStatisticsODBCVer3) {
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 7, "TYPE", SQL_INTEGER, 10, 0, SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 8, "ORDINAL_POSITION", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 12, "PAGES", SQL_INTEGER, 10, 0, SQL_NULLABLE);
...
BOOST_AUTO_TEST_CASE(TestSQLProcedureColumns) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLProcedureColumns(stmt, empty.data(), SQL_NTS, database.data(), SQL_NTS,
                              any.data(), SQL_NTS, any.data(), SQL_NTS);
  } else {
    ret = SQLProcedureColumns(stmt, database.data(), SQL_NTS, empty.data(), SQL_NTS,
                              any.data(), SQL_NTS, any.data(), SQL_NTS);
  }
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 9, "BUFFER_LENGTH", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 10, "DECIMAL_DIGITS", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 11, "NUM_PREC_RADIX", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 15, "SQL_DATA_TYPE", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);

  CheckColumnMetaWithSQLDescribeCol(stmt, 16, "SQL_DATETIME_SUB", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 17, "CHAR_OCTET_LENGTH", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);

  CheckColumnMetaWithSQLDescribeCol(stmt, 18, "ORDINAL_POSITION", SQL_INTEGER, 10, 0,
                                    SQL_NO_NULLS);
...
BOOST_AUTO_TEST_CASE(TestSQLProcedures) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLProcedures(stmt, empty.data(), SQL_NTS, database.data(), SQL_NTS, any.data(),
                        SQL_NTS);
  } else {
    ret = SQLProcedures(stmt, database.data(), SQL_NTS, empty.data(), SQL_NTS, any.data(),
                        SQL_NTS);
  }
  ...
  CheckColumnMetaWithSQLDescribeCol(stmt, 8, "PROCEDURE_TYPE", SQL_INTEGER, 10, 0,
                                    SQL_NULLABLE);
...
BOOST_AUTO_TEST_CASE(TestSQLColumnPrivileges) {
  ...
  if (DATABASE_AS_SCHEMA) {
    ret = SQLColumnPrivileges(stmt, empty.data(), SQL_NTS, database.data(), SQL_NTS,
                              table.data(), SQL_NTS, any.data(), SQL_NTS);
  } else {
    ret = SQLColumnPrivileges(stmt, database.data(), SQL_NTS, empty.data(), SQL_NTS,
                              table.data(), SQL_NTS, any.data(), SQL_NTS);
  }
...
BOOST_AUTO_TEST_CASE(TestColAttributesODBC2ColumnDisplaySize) {
  ...
  // SQL_VARCHAR should have display size TIMESTREAM_SQL_MAX_LENGTH
  callSQLColAttributes(stmt, req1, SQL_COLUMN_DISPLAY_SIZE, TIMESTREAM_SQL_MAX_LENGTH);
}
...
BOOST_AUTO_TEST_CASE(TestColAttributesODBC2ColumnName) {
  ...
  callSQLColAttributes(stmt, req, SQL_COLUMN_NAME, std::string("video_startup_time"));
}
...
BOOST_AUTO_TEST_CASE(TestColAttributesODBC2ColumnPrecision) {
  ...
  // SQL_VARCHAR should have precision TIMESTREAM_SQL_MAX_LENGTH
  callSQLColAttributes(stmt, req1, SQL_COLUMN_PRECISION, TIMESTREAM_SQL_MAX_LENGTH);
}
...
BOOST_AUTO_TEST_CASE(TestColAttributesODBC2ColumnUpdatable) {
  ...
  // only SQL_ATTR_READWRITE_UNKNOWN is returned
  callSQLColAttributes(stmt, req, SQL_COLUMN_UPDATABLE, SQL_ATTR_READWRITE_UNKNOWN);
}
...
BOOST_AUTO_TEST_CASE(TestColAttributesODBC2ColumnTypeName) {
  ...
  callSQLColAttributes(stmt, req, SQL_COLUMN_TYPE_NAME, std::string("TIMESTAMP"));
}
