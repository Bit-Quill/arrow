...
TEST_F(FlightSQLODBCMockTestBase, SQLTablesTestInputData) {
  ...
  SQLRETURN ret =
      SQLTables(this->stmt, catalogName, sizeof(catalogName), schemaName, sizeof(schemaName),
                tableName, sizeof(tableName), tableType, sizeof(tableType));
...
TEST_F(FlightSQLODBCMockTestBase, SQLTablesTestGetMetadataForAllCatalogs) {
  ...
  SQLRETURN ret = SQLTables(this->stmt, SQL_ALL_CATALOGS_W, SQL_NTS, empty, SQL_NTS, empty, SQL_NTS, empty, SQL_NTS);
...
TEST_F(FlightSQLODBCMockTestBase, SQLTablesTestGetMetadataForNamedCatalog) {
  ...
  // TODO Additional fetch returns data for specific table
  validateFetch(this->stmt, SQL_SUCCESS);
  ...
  // Mock server does not support table remarks
...
TEST_F(FlightSQLODBCMockTestBase, SQLTablesTestGetSchemaHasNoData) {
  ...
  // Validate that no schema data is available
...
TEST_F(FlightSQLODBCMockTestBase, SQLTablesTestGetMetadataForAllTables) {
  ...
  SQLRETURN ret = SQLTables(this->stmt, nullptr, SQL_NTS, nullptr, SQL_NTS, SQL_ALL_TABLES_W, SQL_NTS, nullptr, SQL_NTS);

  EXPECT_EQ(ret, SQL_SUCCESS);

  for (size_t i = 0; i < sizeof(tableNames) / sizeof(*tableNames); ++i) {
    // TODO Does not support empty string for no filter
    validateFetch(this->stmt, SQL_SUCCESS);

    std::wstring expectedTableName = std::wstring(tableNames[i]);
    CheckStringColumnW(this->stmt, 1, expectedCatalogName);
    // Mock server does not support table schema
    CheckNullColumnW(this->stmt, 2);
    CheckStringColumnW(this->stmt, 3, expectedTableName);
...
TEST_F(FlightSQLODBCMockTestBase, SQLTablesGetMetadataForTableTypeHasNoData) {
  ...
  SQLWCHAR* typeLists[] = {(SQLWCHAR*)L"'TABLE'", (SQLWCHAR*)L"TABLE,VIEW"};
  SQLRETURN ret = SQL_SUCCESS;

  for (size_t i = 0; i < sizeof(typeLists) / sizeof(*typeLists); ++i) {

    // TODO Mock server doess not support filtering by table type
    ret = SQLTables(this->stmt, nullptr, SQL_NTS, nullptr, SQL_NTS, table, SQL_NTS,
                    typeLists[i], SQL_NTS);
