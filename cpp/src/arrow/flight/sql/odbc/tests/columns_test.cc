...
TYPED_TEST(ColumnsTest, SQLColumnsTestInputData) {
  ...
  EXPECT_EQ(SQL_SUCCESS,
            SQLColumns(stmt, catalog_name, sizeof(catalog_name), schema_name,
...
TEST_F(ColumnsMockTest, SQLDescribeColValidateInput) {
  ...
  EXPECT_EQ(SQL_ERROR, SQLDescribeCol(stmt, bookmark_column, column_name, buf_char_len,
  ...
  // Invalid descriptor index - index out of range
  EXPECT_EQ(SQL_ERROR, SQLDescribeCol(stmt, negative_column, column_name, buf_char_len,
                                      &name_length, &data_type, &column_size,
                                      &decimal_digits, &nullable));
  VerifyOdbcErrorState(SQL_HANDLE_STMT, stmt, kErrorState07009);
