...
TYPED_TEST(StatementTest, TestSQLExecDirectDataQuery) {
  ...
  ASSERT_EQ(SQL_SUCCESS,
            SQLGetData(stmt, 27, SQL_C_WCHAR, &wvarchar_val, buf_len, &ind));
  ...
  ASSERT_EQ(SQL_SUCCESS,
            SQLGetData(stmt, 29, SQL_C_TYPE_DATE, &date_var, buf_len, &ind));
  ...
  ASSERT_EQ(SQL_SUCCESS,
            SQLGetData(stmt, 30, SQL_C_TYPE_DATE, &date_var, buf_len, &ind));
...
TEST_F(StatementRemoteTest, TestSQLExecDirectDataQueryDefaultType) {
  ...
  ASSERT_EQ(SQL_SUCCESS,
            SQLGetData(stmt, 21, SQL_C_DEFAULT, &double_val, buf_len, &ind));
  // Get minimum negative double value
  EXPECT_EQ(-std::numeric_limits<SQLDOUBLE>::max(), double_val);

  ASSERT_EQ(SQL_SUCCESS,
            SQLGetData(stmt, 22, SQL_C_DEFAULT, &double_val, buf_len, &ind));
  EXPECT_EQ(std::numeric_limits<SQLDOUBLE>::max(), double_val);
  ...
  buf_len = wchar_size * 2;
  ASSERT_EQ(SQL_SUCCESS,
            SQLGetData(stmt, 26, SQL_C_DEFAULT, &wchar_val2, buf_len, &ind));
...
TYPED_TEST(StatementTest, TestSQLBindColDataQuery) {
  ...
  ASSERT_EQ(SQL_SUCCESS,
            SQLBindCol(stmt, 9, SQL_C_SLONG, &slong_val_min, buf_len, &ind));
  ...
  ASSERT_EQ(SQL_SUCCESS,
            SQLBindCol(stmt, 27, SQL_C_WCHAR, &wvarchar_val, buf_len, &ind));
...
TYPED_TEST(StatementTest, TestSQLExtendedFetchRowFetching) {
  ...
  ASSERT_EQ(SQL_SUCCESS, SQLSetStmtAttr(stmt, SQL_ROWSET_SIZE,
                                        reinterpret_cast<SQLPOINTER>(rows), 0));
