...
class BlockingQueue {
  ...
  void AddProducer(Supplier supplier) {
    ...
    threads_.emplace_back([=] {
      while (!closed_) {
        ...
        std::unique_lock<arrow::util::Mutex> unique_lock(mtx_);
 ...
 private:
  bool WaitUntilCanPushOrClosed(std::unique_lock<arrow::util::Mutex>& unique_lock) {
    not_full_.wait(unique_lock,
                   [this]() { return closed_ || buffer_size_ != capacity_; });
    return !closed_;
  }

  bool WaitUntilCanPopOrClosed(std::unique_lock<arrow::util::Mutex>& unique_lock) {
