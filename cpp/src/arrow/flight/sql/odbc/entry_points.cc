...
SQLRETURN SQL_API SQLNativeSql(SQLHDBC connectionHandle, SQLWCHAR* inStatementText,
  ...
  return ODBC::ODBCConnection::ExecuteWithDiagnostics(connectionHandle, SQL_ERROR, [=]() {
    throw driver::odbcabstraction::DriverException("SQLNativeSqlW is not implemented",
                                                   "IM001");
    return SQL_ERROR;
  });
}
