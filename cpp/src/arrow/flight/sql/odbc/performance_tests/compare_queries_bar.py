...
def main():
    ...
    queries = {
        ...
        "CountByVendor": f'SELECT vendor_id, COUNT(*) AS num_trips FROM "{args.schema}"."{args.table}" GROUP BY vendor_id',
        "TipStatsByPayment": f'SELECT payment_type, AVG(tip_amount) AS avg_tip, MAX(tip_amount) AS max_tip FROM "{args.schema}"."{args.table}" GROUP BY payment_type',
        "FareGreater50": f'SELECT * FROM "{args.schema}"."{args.table}" WHERE fare_amount > 50 LIMIT 500',
        "TripDistance5to10": f'SELECT * FROM "{args.schema}"."{args.table}" WHERE trip_distance BETWEEN 5 AND 10 LIMIT 500',
        "PassengerCount2": f'SELECT * FROM "{args.schema}"."{args.table}" WHERE passenger_count = 2 LIMIT 500',
        "CashPayments": f'SELECT * FROM "{args.schema}"."{args.table}" WHERE payment_type = \'CSH\' LIMIT 500',
        "OrderByFareDesc": f'SELECT * FROM "{args.schema}"."{args.table}" ORDER BY fare_amount DESC LIMIT 100',
        "OrderByTripDistance": f'SELECT * FROM "{args.schema}"."{args.table}" ORDER BY trip_distance DESC LIMIT 100',
        "DistinctVendors": f'SELECT DISTINCT vendor_id FROM "{args.schema}"."{args.table}"',
        "DistinctPayments": f'SELECT DISTINCT payment_type FROM "{args.schema}"."{args.table}"',
    ...
    for label, query in queries.items():
        print(f"\nRunning {label}...")
        # If query is None, use table_read_test.py default LIMIT query
    ...
    driver_colors = {"a": "#1f77b4", "b": "#ff7f0e"}
